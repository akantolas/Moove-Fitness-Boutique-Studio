export const el = {
  meta: {
    studioTitle: '{{owner}} · {{tagline}} Βόλος',
    posingTitle: '{{brand}} — Online posing coaching',
    posingAboutTitle: '{{brand}} — Σχετικά',
  },
  lang: {
    el: 'Ελληνικά',
    en: 'English',
    switch: 'Γλώσσα',
  },
  nav: {
    home: 'Αρχική',
    classes: 'Μαθήματα',
    about: 'Σχετικά',
    contact: 'Επικοινωνία',
    programs: 'Προγράμματα',
    packages: 'Πακέτα',
    main: 'Κύρια πλοήγηση',
    mobile: 'Κινητή πλοήγηση',
    openMenu: 'Άνοιγμα μενού',
    closeMenu: 'Κλείσιμο μενού',
  },
  common: {
    book: 'Κράτηση',
    bookClass: 'Κράτηση μαθήματος',
    bookPosing: 'Κράτηση posing session',
    bookPosingSession: 'Κράτηση συνεδρίας',
    bookSpot: 'Κλείσε θέση',
    instagram: 'Instagram',
    facebook: 'Facebook',
    googleMaps: 'Google Maps',
    email: 'Email',
    poweredBy: 'Powered by',
    copyright: 'Με επιφύλαξη παντός δικαιώματος.',
    learnMore: 'Δες περισσότερα',
    send: 'Αποστολή',
    level: 'Επίπεδο',
    duration: 'Διάρκεια',
    volos: 'Βόλος',
    close: 'Κλείσιμο',
    viewLarger: 'Μεγέθυνση: {{alt}}',
  },
  header: {
    bookPosingShort: 'Κράτηση',
  },
  footer: {
    studioBlurb: 'Pilates & Reformer studio στον Βόλο, υπό την καθοδήγηση της {{owner}}.',
    posingBlurb: 'Online posing coaching για διαγωνισμούς bodybuilding — από την {{coach}}.',
    posingSubtitle: 'Η Ακαδημία Posing',
    backToStudio: '← Επιστροφή στο {{name}} studio',
    nav: 'Πλοήγηση',
    details: 'Στοιχεία',
    social: 'Social',
    moveAndPose: 'Move & Pose',
    studioClasses: 'Μαθήματα studio',
  },
  home: {
    hero: {
      title: 'Καλώς ήρθες στο Moove Fitness Boutique Studio',
      titleAccent: '',
      body: 'Το Moove Fitness Boutique Studio, που δημιούργησε η Μάγδα Σαμαρά, είναι ένας boutique χώρος άσκησης αφιερωμένος στην ποιοτική και ασφαλή εκγύμναση.',
      body2:
        'Όλα τα μαθήματα πραγματοποιούνται σε ολιγομελή τμήματα και διδάσκονται με έμφαση στη σωστή τεχνική, την εξατομικευμένη καθοδήγηση και τη δημιουργία μιας μοναδικής εμπειρίας άσκησης.',
      heroImageAlt: 'Μάθημα Pilates στο Moove',
      location: 'Κοραή 137 - Δ. Γεωργιάδου 106Γ · Βόλος',
      viewClasses: 'Δες μαθήματα',
    },
    stats: {
      reformer: 'Reformer',
      reformerLabel: 'Σύγχρονος εξοπλισμός',
      groups: 'Μικρά γκρουπ',
      groupsLabel: 'Προσωπική καθοδήγηση',
      volos: 'Βόλος',
      volosLabel: 'Κοραή 137 - Δ. Γεωργιάδου 106Γ',
    },
    approach: {
      eyebrow: 'Η προσέγγιση',
      title: 'Προσωπική καθοδήγηση',
      body: 'Σε μικρό studio δεν χάνεσαι στο πλήθος. Κάθε μάθημα σχεδιάζεται με προσοχή στη τεχνική, στη διόρθωση και στις ανάγκες του κάθε ασκούμενου.',
      items: [
        {
          title: 'Μικρά γκρουπ',
          body: 'Λίγοι ασκούμενοι ανά μάθημα, ώστε η καθοδήγηση να είναι πραγματικά προσωπική — με διόρθωση και σαφείς οδηγίες.',
        },
        {
          title: 'Ήρεμος χώρος',
          body: 'Καθαρό studio, σύγχρονος εξοπλισμός και ήσυχη ατμόσφαιρα — μακριά από τον θόρυβο των μεγάλων χώρων.',
        },
        {
          title: 'Σταδιακή πρόοδος',
          body: 'Δύναμη, ευελιξία και σταθερότητα χτίζονται με μέθοδο και συνέπεια, όχι με βιασύνη.',
        },
      ],
    },
    classes: {
      eyebrow: 'Μαθήματα',
      title: 'Τα μαθήματα',
      body: 'Επιλέξτε το πρόγραμμα που σας ταιριάζει — η καθοδήγηση προσαρμόζεται στο επίπεδό σας.',
      viewProgram: 'Δες το πρόγραμμα',
      imageAlt: 'Pilates στο Moove',
    },
    cta: {
      eyebrow: 'Έλα να δοκιμάσεις',
      title: 'Θέλεις να κλείσεις θέση;',
      body: 'Κλείστε θέση online ή επικοινωνήστε μαζί μας για οποιαδήποτε απορία πριν την πρώτη σας επίσκεψη.',
    },
  },
  about: {
    eyebrow: 'Σχετικά',
    description: 'Ιδρύτρια του {{name}} · Γυμνάστρια · Δημιουργός της Move & Pose',
    experienceTitle: 'Βιογραφικό',
    paragraphs: [
      'Η Μάγδα Σαμαρά είναι απόφοιτος του Τμήματος Επιστήμης Φυσικής Αγωγής και Αθλητισμού (ΤΕΦΑΑ) του Πανεπιστημίου Θεσσαλίας και ιδρύτρια του Moove Fitness Boutique Studio στον Βόλο. Διαθέτει πολυετή εμπειρία στον χώρο της άσκησης και της ευεξίας, έχοντας εκπαιδεύσει εκατοντάδες ασκούμενους μέσα από εξατομικευμένες και ομαδικές προπονήσεις.',
      'Εξειδικεύεται στο Pilates Mat & Reformer, στη λειτουργική προπόνηση, στην άσκηση για ειδικούς πληθυσμούς, καθώς και στην αγωνιστική προετοιμασία και το posing αθλητών fitness. Παράλληλα, είναι δημιουργός της Move & Pose – The Posing Academy, της πρώτης εξειδικευμένης ακαδημίας posing στην Ελλάδα.',
      'Ως ενεργή αθλήτρια bodybuilding και fitness, έχει εκπροσωπήσει την Ελλάδα και τον Βόλο σε διεθνείς διοργανώσεις, κατακτώντας σημαντικές διακρίσεις.',
      'Με γνώμονα τη συνεχή εξέλιξη και τη δια βίου μάθηση, στόχος της είναι να προσφέρει εκπαίδευση υψηλού επιπέδου, βασισμένη στην επιστημονική γνώση, τη σωστή τεχνική και την εξατομικευμένη προσέγγιση, βοηθώντας κάθε ασκούμενο να εξελιχθεί με ασφάλεια και αυτοπεποίθηση.',
    ],
    highlights: [
      'ΤΕΦΑΑ · Πανεπιστήμιο Θεσσαλίας',
      'Pilates Mat & Reformer',
      'Move & Pose Academy',
      'Βόλος',
    ],
    studioTitle: 'Το studio',
    studioBody:
      'Φυσικό φως, καθαρές γραμμές και σύγχρονος εξοπλισμός — στην Κοραή 137, λίγα λεπτά από το κέντρο του Βόλου.',
    gallery: {
      pilates: 'Μάθημα Pilates',
      reformer: 'Reformer',
    },
  },
  contact: {
    eyebrow: 'Επικοινωνία',
    title: 'Πού βρισκόμαστε',
    description: 'Για κράτηση πήγαινε online. Για οτιδήποτε άλλο, γράψε ή πάρε τηλέφωνο.',
    detailsTitle: 'Στοιχεία επικοινωνίας',
    address: 'Διεύθυνση',
    phone: 'Τηλέφωνο',
    hours: 'Ώρες',
    hoursValue: 'Δευ–Παρ: 07:00–21:00 · Σαβ: 09:00–15:00',
    openMaps: 'Άνοιγμα στο Google Maps →',
    messageTitle: 'Στείλε μήνυμα',
    sent: "Το πήραμε! Θα σ' απαντήσουμε το συντομότερο δυνατό.",
    sending: 'Αποστολή…',
    sendError: 'Δεν στάλθηκε το μήνυμα. Δοκίμασε ξανά ή πάρε τηλέφωνο.',
    sendUnavailable: 'Η αποστολή δεν είναι διαθέσιμη αυτή τη στιγμή. Πάρε τηλέφωνο ή γράψε στο info@moovefitness.gr.',
    name: 'Ονοματεπώνυμο',
    namePlaceholder: 'π.χ. Μαρία Παπαδοπούλου',
    emailPlaceholder: 'το email σου',
    message: 'Μήνυμα',
    messagePlaceholder: 'Γράψε εδώ ό,τι θες να ρωτήσεις...',
    consentBefore: 'Συμφωνώ με την επεξεργασία των στοιχείων μου για απάντηση στο αίτημά μου, σύμφωνα με την',
    consentAfter: '.',
  },
  classes: {
    eyebrow: 'Μαθήματα',
    title: 'Πρόγραμμα & τύποι',
    description: 'Εβδομαδιαίο group πρόγραμμα. Για κράτηση, χρησιμοποιήστε το online σύστημα.',
    weekTitle: 'Καλοκαιρινό εβδομαδιαίο πρόγραμμα',
    weekRange: 'Δευτέρα – Σάββατο',
    detailsEyebrow: 'Λεπτομέρειες',
    typesTitle: 'Τύποι μαθημάτων',
    items: [
      {
        title: 'Pilates Mat',
        level: 'Όλα τα επίπεδα',
        duration: '45–50 λεπτά',
        desc: 'Βελτιώστε τη δύναμη, τη στάση του σώματος και την ευλυγισία σας μέσα από το κλασικό Pilates στο στρώμα, με ασκήσεις κατάλληλες για κάθε επίπεδο.',
      },
      {
        title: 'Pilates Reformer & Equipment',
        level: 'Όλα τα επίπεδα',
        duration: '45–50 λεπτά',
        desc: 'Ανακαλύψτε τα οφέλη του Pilates με εξειδικευμένο εξοπλισμό, ενισχύοντας τη δύναμη, τη σταθερότητα, την ευθυγράμμιση και τον έλεγχο του σώματος.',
      },
      {
        title: 'Προγράμματα Fitness Κλειστού Χώρου',
        level: 'Όλα τα επίπεδα',
        duration: '45–50 λεπτά',
        desc: 'Επιλέξτε δυναμικά προγράμματα που συνδυάζουν ενδυνάμωση, αντοχή και ευεξία, προσαρμοσμένα στις ανάγκες και τους στόχους σας.',
      },
      {
        title: 'Ιδιαίτερα Μαθήματα Χορού & Αγωνιστικής Προετοιμασίας',
        level: 'Όλα τα επίπεδα',
        duration: 'Ιδιαίτερο',
        desc: 'Εξατομικευμένη καθοδήγηση για τη βελτίωση της τεχνικής, της έκφρασης, της κίνησης και της συνολικής σας παρουσίας, είτε ο στόχος σας είναι η προσωπική εξέλιξη είτε η αγωνιστική προετοιμασία.',
      },
      {
        title: 'Άσκηση κατά την Εγκυμοσύνη & Μετά τον Τοκετό',
        level: 'Ειδικός πληθυσμός',
        duration: '45–50 λεπτά',
        desc: 'Ασφαλή και εξειδικευμένα προγράμματα που υποστηρίζουν το σώμα κατά τη διάρκεια της εγκυμοσύνης και βοηθούν στην ομαλή αποκατάσταση μετά τον τοκετό.',
      },
      {
        title: 'Άσκηση για την Τρίτη Ηλικία',
        level: 'Ειδικός πληθυσμός',
        duration: '45 λεπτά',
        desc: 'Προγράμματα σχεδιασμένα για τη διατήρηση της δύναμης, της ισορροπίας, της κινητικότητας και της ανεξαρτησίας, συμβάλλοντας σε μια καλύτερη ποιότητα ζωής.',
      },
      {
        title: 'Glutes & Abs',
        level: 'Όλα τα επίπεδα',
        duration: '45–50 λεπτά',
        desc: 'Στοχευμένη προπόνηση γλουτών και κοιλιακών — δύναμη, σταθερότητα και έλεγχος τεχνικής σε δυναμικά μαθήματα.',
      },
    ],
  },
  programs: {
    processing: {
      eyebrow: 'Σύντομα διαθέσιμα',
      title: 'Τα προγράμματα είναι υπό επεξεργασία',
      body:
        'Ετοιμάζουμε νέα προγράμματα προπόνησης. Η σελίδα αγοράς είναι προσωρινά κλειστή — θα είναι διαθέσιμη σύντομα.',
      note:
        'Αν έχεις ήδη αγοράσει πρόγραμμα, το link πρόσβασης στο email σου συνεχίζει να λειτουργεί κανονικά.',
      cta: 'Επικοινωνία',
      backHome: 'Πίσω στην αρχική',
    },
    eyebrow: 'Peach training system',
    title: 'Χτίσε γλουτούς με πρόγραμμα.',
    titleAccent: 'Όχι στην τύχη.',
    description:
      'Δομημένα προγράμματα γλουτών, σχεδιασμένα για να ξέρεις ακριβώς τι κάνεις σε κάθε προπόνηση — από την ενεργοποίηση μέχρι το finisher.',
    heroBadge: 'Εξατομικευμένη δομή · On-demand πρόσβαση',
    heroCta: 'Δες τα προγράμματα',
    heroImageAlt: 'Preview του Peach training program',
    proof: {
      structured: 'Πλάνο ανά προπόνηση',
      coach: 'Από τη Μάγδα Σαμαρά',
      access: 'Private πρόσβαση μετά την πληρωμή',
    },
    purchase: {
      eyebrow: 'Ξεκίνα εδώ',
      title: 'Ασφαλές checkout',
      body: 'Βάλε το email σου και ολοκλήρωσε με ασφάλεια την πληρωμή στο Stripe.',
      paymentMethod: 'Ασφαλής πληρωμή με κάρτα μέσω Stripe',
    },
    catalog: {
      eyebrow: 'Διάλεξε το πλάνο σου',
      title: 'Τέσσερις επιλογές. Ένας ξεκάθαρος στόχος.',
      body: 'Κάθε αγορά περιλαμβάνει όλες τις προπονήσεις του προγράμματος και προσωπικό private link.',
      selectorLabel: 'Διάλεξε πακέτο',
      selectorHint: 'Υπάρχουν 4 διαθέσιμα πακέτα — πάτησε για να δεις λεπτομέρειες.',
    },
    nutritionUpsell: {
      eyebrow: 'Προαιρετική προσθήκη',
      title: 'Πρόσθεσε προσωποποιημένη διατροφή',
      body: 'Εβδομαδιαίο πλάνο γευμάτων σε PDF, με βάση το ερωτηματολόγιό σου.',
      bullets: [
        'Πλάνο σε PDF στο email σου μετά την πληρωμή',
        'Συμπληρώνεις σύντομο ερωτηματολόγιο στο checkout',
        'Διαθέσιμο μόνο με αγορά προγράμματος',
      ],
      priceLabel: '+{{price}}€',
      checkoutHint: 'Ενεργοποιείται στο βήμα πληρωμής — τσέκαρε την επιλογή στο checkout.',
    },
    collections: {
      collection: {
        eyebrow: 'Peach Collection',
        title: 'Η βάση για να ξεκινήσεις σωστά',
        body: 'Τρία focused workouts για τεχνική, έλεγχο και σταδιακή πρόοδο.',
      },
      build: {
        eyebrow: 'Peach Build',
        title: 'Για ένταση, όγκο και δύναμη',
        body: 'Ένα 6-week intensive split για πιο απαιτητική προπόνηση γλουτών.',
      },
      sculpt: {
        eyebrow: 'Peach Sculpt',
        title: 'Shape · Definition · Symmetry',
        body: '8 εβδομάδες με υψηλότερο volume, supersets και στοχευμένο glute pump για ορισμό χωρίς απώλεια δύναμης.',
      },
    },
    benefits: {
      foundation: 'Βάση δύναμης & τεχνικής',
      shape: 'Σχήμα & σταθερότητα',
      sculpt: 'Glute shape & activation',
      definition: 'Definition & symmetry',
      symmetry: 'Pump & symmetry',
      strength: 'Heavy glute strength',
      unilateral: 'Unilateral control',
      posterior: 'Posterior-chain focus',
      volume: 'Volume & pump',
      conditioning: 'Conditioning & burn',
      complete: 'Ολοκληρωμένο Peach system',
    },
    outcomes: {
      foundation: 'Χτίσε σωστή τεχνική και δύναμη στα βασικά patterns.',
      shape: 'Δούλεψε έλεγχο, σταθερότητα και καθαρή εκτέλεση.',
      sculpt: 'Δώσε έμφαση σε activation και ολοκληρωμένο glute work.',
      definition: 'Μέγιστος ορισμός γλουτών με supersets, unilateral work και pump sessions.',
      symmetry: 'Αύξησε ενεργοποίηση και συμμετρία με unilateral pump work.',
      strength: 'Ανέβασε ένταση με structured heavy glute training.',
      unilateral: 'Χτίσε δύναμη και έλεγχο σε κάθε πλευρά ξεχωριστά.',
      posterior: 'Δούλεψε οπίσθια αλυσίδα με στόχο πραγματική πρόοδο.',
      volume: 'Πρόσθεσε όγκο και pump με πιο υψηλό training volume.',
      conditioning: 'Αύξησε conditioning και θερμιδική κατανάλωση με μεταβολικούς κύκλους και high-rep glute work.',
      complete: 'Από τη σωστή βάση μέχρι δύναμη, όγκο, ορισμό και conditioning — και οι 12 προπονήσεις σε μία συλλογή.',
    },
    featured: 'Δημοφιλής επιλογή',
    includes: {
      eyebrow: 'Μέσα στο πρόγραμμα',
      title: 'Ξεκάθαρη καθοδήγηση σε κάθε session',
      items: [
        'Προθέρμανση και activation πριν από τα βασικά sets',
        'Ασκήσεις, σετ, επαναλήψεις και πρακτικές σημειώσεις',
        'Load plan για προοδευτική επιβάρυνση',
        'Αποθεραπεία με δομημένες διατάσεις σε κάθε προπόνηση',
      ],
    },
    delivery: {
      eyebrow: 'Πώς λειτουργεί',
      title: 'Από την επιλογή στην προπόνηση',
      steps: [
        { title: 'Διάλεξε πρόγραμμα', body: 'Σύγκρινε στόχο, επίπεδο και διάρκεια.' },
        { title: 'Ολοκλήρωσε πληρωμή', body: 'Θα λάβεις ασφαλείς επιλογές πληρωμής στο email σου.' },
        { title: 'Προπονήσου', body: 'Μετά την επιβεβαίωση σού στέλνουμε το προσωπικό σου link.' },
      ],
    },
    sculptGuidelines: {
      eyebrow: 'Διάβασε πριν προπονηθείς',
      title: 'Γενικές οδηγίες προγράμματος',
      methodsTitle: 'Μέθοδοι προπόνησης',
      applicationLabel: 'Στο Peach Sculpt',
      methods: [
        {
          title: 'Supersets',
          body: 'Μέθοδος που συνδυάζει δύο ασκήσεις τη μία μετά την άλλη, με ελάχιστη ή καθόλου ξεκούραση ανάμεσά τους. Αυξάνει την ένταση, την αντοχή και τον όγκο προπόνησης σε λιγότερο χρόνο.',
          application:
            'Στο Peach Sculpt, τα supersets χρησιμοποιούνται στρατηγικά για αύξηση μεταβολικού stress και καλύτερη ενεργοποίηση γλουτών, διατηρώντας την ποιότητα της προπόνησης.',
        },
        {
          title: 'Drop Sets',
          body: 'Προχωρημένη τεχνική έντασης: μείωση βάρους αμέσως μετά τις επιθυμητές επαναλήψεις και συνέχιση της άσκησης χωρίς διάλειμμα. Επεκτείνει το σετ πέρα από την κανονική κόπωση και αυξάνει την ενεργοποίηση ινών και το μεταβολικό stress.',
          application:
            'Στο Peach Sculpt, τα drop sets προορίζονται για το τελευταίο σετ επιλεγμένων ασκήσεων απομόνωσης, για μέγιστο ορισμό με ελάχιστο stress στις αρθρώσεις.',
        },
      ],
      whyTitle: 'Γιατί χρησιμοποιούμε αυτές τις μεθόδους',
      whyBody:
        'Το Peach Sculpt σχεδιάστηκε για μέγιστο ορισμό γλουτών διατηρώντας τη μυϊκή μάζα. Supersets και drop sets αυξάνουν την πυκνότητα προπόνησης, βελτιώνουν την αντοχή και διεγείρουν μυϊκή ανάπτυξη χωρίς να μεγαλώνουν άσκοπα τη διάρκεια. Προορίζονται για intermediate–advanced αθλητές με σωστή τεχνική.',
      techniquesTitle: 'Τεχνικές αναφοράς',
      exampleLabel: 'Παράδειγμα',
      noteLabel: 'Σημείωση',
      techniques: [
        {
          title: 'Ξεκούραση',
          body: 'Η ξεκούραση είναι ο χρόνος ανάκαμψης πριν το επόμενο σετ ή την επόμενη άσκηση.',
          example: 'Rest: 30″ = ξεκούραση 30 δευτερόλεπτα πριν συνεχίσεις.',
        },
        {
          title: 'Rest-Pause',
          body: 'Προχωρημένη τεχνική έντασης: αφού φτάσεις τις επιθυμητές επαναλήψεις, ξεκουράσου σύντομα και συνέχισε με το ίδιο βάρος για λίγες ακόμη επαναλήψεις.',
          example:
            'Παράδειγμα — Barbell Hip Thrust:\n80 kg × 10 επαναλήψεις\nΞεκούραση 15″ → 3–4 επιπλέον επαναλήψεις\nΞεκούραση 15″ → 2–3 επιπλέον επαναλήψεις',
          note: 'Ολόκληρη η ακολουθία μετράει ως ένα σετ.',
        },
        {
          title: 'Drop Set',
          body: 'Προχωρημένη μέθοδος: αμέσως μετά τις προβλεπόμενες επαναλήψεις μείωσε το βάρος και συνέχισε την άσκηση χωρίς διάλειμμα.',
          example:
            'Παράδειγμα — Seated Hip Abduction:\n20 επαναλήψεις × 60 kg\nΜείωση σε 45 kg → 12 επαναλήψεις\nΜείωση σε 30 kg → όσες ποιοτικές επαναλήψεις μπορείς',
          note: 'Ολόκληρη η ακολουθία μετράει ως ένα drop set.',
        },
      ],
    },
    durationLabel: 'Διάρκεια',
    priceLabel: 'Τιμή',
    vatIncluded: 'Συμπεριλαμβάνεται ΦΠΑ',
    workoutsLabel: 'προπονήσεις',
    levels: {
      beginner: 'Αρχάριοι',
      intermediate: 'Μεσαίο',
      intermediateAdvanced: 'Μεσαίο / Προχωρημένο',
      allLevels: 'Όλα τα επίπεδα',
    },
    durations: {
      weeks4: '4 εβδομάδες',
      weeks6: '6 εβδομάδες',
      weeks4x3: '4 εβδομάδες · 3x/εβδομάδα',
      weeks8x45: '8 εβδομάδες · 4–5x/εβδομάδα',
      complete: 'Πλήρης συλλογή',
    },
    items: {
      peach_start_bundle: {
        title: 'Peach Start',
        desc: 'Τρεις προπονήσεις για τεχνική, ενεργοποίηση και σταδιακό χτίσιμο δύναμης στα βασικά glute patterns.',
      },
      peach_build: {
        title: 'Peach Build',
        desc: 'Τέσσερις προπονήσεις δύναμης και όγκου με heavy lifts, unilateral work, posterior chain και pump.',
      },
      peach_sculpt: {
        title: 'Peach Sculpt',
        desc: 'Πέντε προπονήσεις με supersets, pump, conditioning και τεχνικές έντασης για shape, definition και symmetry.',
      },
      peach_complete: {
        title: 'Complete Collection',
        desc: 'Και οι 12 προπονήσεις των Peach Start, Build και Sculpt σε ένα ολοκληρωμένο training system.',
      },
    },
    order: {
      emailLabel: 'Email για παραλαβή link',
      emailPlaceholder: 'το-email@example.com',
      hint: 'Μετά την επιβεβαίωση της πληρωμής θα λάβεις email με το private link του προγράμματός σου. Δεν χρειάζεται λογαριασμός.',
      cta: 'Συνέχισε σε ασφαλή πληρωμή',
      submitting: 'Μετάβαση στο Stripe…',
      successTitle: 'Έλεγξε το email',
      successBody: 'Στείλαμε email με συνδέσμους πληρωμής. Μετά την πληρωμή θα λάβεις link πρόσβασης.',
      refLabel: 'Κωδικός παραγγελίας',
      invalidEmail: 'Βάλε έγκυρο email.',
      nutritionAddon: 'Προσθήκη προσωποποιημένης διατροφής (+{{price}}€)',
      nutritionAddonHint: 'Διαθέσιμη μόνο με αγορά προγράμματος.',
      nutritionQuestionnaireTitle: 'Ερωτηματολόγιο διατροφής',
      nutritionQuestionnaireHint: 'Συμπλήρωσε τα στοιχεία για να δημιουργηθεί το προσωποποιημένο πλάνο σου.',
      cancelNutrition: 'Ακύρωση προσθήκης διατροφής',
      errors: {
        order_failed: 'Η παραγγελία απέτυχε. Δοκίμασε ξανά.',
        rate_limit: 'Πολλές παραγγελίες — δοκίμασε αργότερα.',
        invalid_email: 'Μη έγκυρο email.',
        invalid_program: 'Μη έγκυρο πρόγραμμα.',
        stripe_unavailable: 'Η ασφαλής πληρωμή δεν είναι διαθέσιμη αυτή τη στιγμή. Δοκίμασε ξανά αργότερα.',
        stripe_not_configured: 'Το Stripe δεν είναι ρυθμισμένο τοπικά. Πρόσθεσε STRIPE_SECRET_KEY στο .env.local και κάνε restart το vercel dev.',
      },
    },
    paymentSuccess: {
      title: 'Η πληρωμή ολοκληρώθηκε',
      body: 'Θα λάβεις email με το link πρόσβασης στο πρόγραμμα σύντομα.',
      paid: 'Η πληρωμή επιβεβαιώθηκε — έλεγξε το email για το link πρόσβασης.',
    },
    access: {
      eyebrow: 'Το πρόγραμμά σου',
      loading: 'Φόρτωση…',
      notFoundTitle: 'Δεν βρέθηκε πρόσβαση',
      notFoundBody: 'Το link είναι άκυρο ή η πληρωμή δεν έχει ολοκληρωθεί.',
      errorSupport: 'Χρειάζεσαι βοήθεια; Επικοινώνησε με το Moove Fitness',
      privateNote: 'Προσωπική χρήση — μην μοιράζεσαι αυτό το link.',
      privateAccess: 'Ιδιωτική πρόσβαση',
      sets: 'Σετ',
      loadVideo: 'Παίξε video',
      metaDuration: 'Διάρκεια',
      metaGoal: 'Στόχος',
      metaProgress: 'Σημείωση προόδου',
      overview: 'Επισκόπηση προγράμματος',
      programPlan: 'Ο χάρτης της προπόνησής σου',
      workout: 'Προπόνηση',
      chooseWorkout: 'Διάλεξε προπόνηση',
      includedWorkouts: 'Οι προπονήσεις σου',
      groups: {
        start: 'Peach Start',
        build: 'Peach Build',
        sculpt: 'Peach Sculpt',
        other: 'Προπόνηση',
      },
      todaysPlan: 'Το σημερινό πλάνο',
      sections: 'ενότητες',
      exercises: 'ασκήσεις',
      startProgram: 'Ξεκίνα προπόνηση',
      sectionNavigation: 'Πλοήγηση ενοτήτων προγράμματος',
      openSection: 'Άνοιγμα ενότητας',
      closeSection: 'Κλείσιμο ενότητας',
      exercise: 'Άσκηση',
      instruction: 'Οδηγία',
      prescription: {
        repetitions: 'Επαναλήψεις',
        setsRepetitions: 'Σετ × επαναλήψεις',
        sets: 'Σετ',
        time: 'Χρόνος',
        distance: 'Απόσταση',
        steps: 'Βήματα',
        effort: 'Εκτέλεση',
        perLeg: 'πόδι',
        perSide: 'πλευρά',
      },
    },
    admin: {
      pendingTitle: 'Εκκρεμείς πληρωμές προγραμμάτων',
      noPending: 'Δεν υπάρχουν εκκρεμείς πληρωμές προγραμμάτων.',
      recentPaidTitle: 'Πρόσφατες πληρωμένες παραγγελίες',
      noPaid: 'Δεν υπάρχουν πρόσφατες πληρωμένες παραγγελίες.',
      confirmBody: 'Επιβεβαίωση πληρωμής (PayPal/Revolut) — θα σταλεί email με link πρόσβασης.',
      resendAccess: 'Επανάληψη email πρόσβασης',
      resending: 'Αποστολή…',
    },
  },
  schedule: {
    prevDay: 'Προηγούμενη ημέρα',
    nextDay: 'Επόμενη ημέρα',
    daysAria: 'Ημέρες εβδομάδας',
    program: 'Πρόγραμμα',
    lesson: 'μάθημα',
    lessons: 'μαθήματα',
    swipeHint: 'Σύρε για άλλη μέρα · ',
    counter: '{{current}} από {{total}}',
    days: {
      mon: { label: 'Δευτέρα', short: 'ΔΕΥ' },
      tue: { label: 'Τρίτη', short: 'ΤΡΙ' },
      wed: { label: 'Τετάρτη', short: 'ΤΕΤ' },
      thu: { label: 'Πέμπτη', short: 'ΠΕΜ' },
      fri: { label: 'Παρασκευή', short: 'ΠΑΡ' },
      sat: { label: 'Σάββατο', short: 'ΣΑΒ' },
    },
  },
  reviews: {
    eyebrow: 'Κριτικές',
    title: 'Τι λένε οι ασκούμενοι',
    mapsHint: 'Δες τι γράφουν στο Google Maps —',
    viewProfile: 'δείτε το προφίλ μας',
    onGoogle: '/ 5 στο Google',
    reviewCount: 'κριτικές',
    starsAria: '{{rating}} στα 5 αστέρια',
    allOnMaps: 'Όλες οι κριτικές στο Google Maps',
  },
  reviewsFallback: {
    items: [
      {
        quote:
          'Ήσυχο studio, καταλαβαίνουν τι κάνεις. Σε λίγες εβδομάδες ένιωσα διαφορά στη στάση μου.',
        name: 'Μαρία Κ.',
      },
      {
        quote:
          'Έρχομαι εδώ και καιρό. Μικρά γκρουπ, προσοχή στη λεπτομέρεια, δεν αφήνουν κανέναν να κάνει λάθος χωρίς να το διορθώσουν.',
        name: 'Νίκος Π.',
      },
    ],
    apiKeyHint:
      'Για να φορτώνουν αυτόματα οι κριτικές: Vercel + GOOGLE_PLACES_API_KEY (βλ. .env.example).',
    tempHint:
      'Προσωρινά δείχνουμε ενδεικτικά σχόλια — όλες οι κριτικές είναι στο Google.',
    legal:
      'Οι κριτικές από το Google εμφανίζονται σύμφωνα με τους όρους της Google και μερικές φορές δεν είναι διαθέσιμες μέσω API. Όλα τα σχόλια μένουν πάντα στο',
    mapsProfile: 'προφίλ στο Google Maps',
  },
  posing: {
    hero: {
      title: 'Online posing coaching για bodybuilding shows',
      body: 'Διδακτικές συνεδρίες one-on-one για στάσεις, stage presence και confidence στην πλάτφορμα — εντελώς ξεχωριστά από το Pilates studio.',
      book: 'Κλείσε συνεδρία',
      viewPackages: 'Δες τα πακέτα',
      openCal: 'Άνοιγμα Cal.com',
      byCoach: 'by {{coach}}',
      chips: ['1:1 coaching', 'Stage presence', 'Greek & English', 'Email confirmation'],
    },
    about: {
      eyebrow: 'Σχετικά με εμάς',
      headline: 'Είσαι αρχάριος ή επαγγελματίας στον χώρο του fitness;',
      lead: 'Είμαστε εδώ για να εξασφαλίσουμε την καλύτερη έκδοση της απόδοσής σου.',
      paragraphs: [
        'Με πτυχίο BSc στην Επιστήμη Φυσικής Αγωγής & Αθλητισμού και μέσα από γνώση κινησιολογίας, η Head Coach μας θα σε καθοδηγήσει βήμα-βήμα στο posing journey σου με όλα τα απαραίτητα tips & tricks ώστε το physique σου να ξεχωρίζει στη σκηνή!',
        'Ως επαγγελματίας χοροπαιδαγωγός, πρωταθλήτρια, international adjudicator & fitness athlete, η Head Coach μας θα σου δώσει όλα τα secrets που χρειάζεσαι για να προσθέσεις character & charisma στην απόδοσή σου!',
      ],
      closing: 'Δούλεψε μαζί μας & φέρε τη μαγεία στη σκηνή!',
      viewPackages: 'Δες τα πακέτα',
      bookSession: 'Κλείσε συνεδρία',
      studioNote:
        'Το Move & Pose είναι ξεχωριστή υπηρεσία από τα μαθήματα του Moove studio — online και live posing coaching για bodybuilding shows.',
      backToPosing: '← Επιστροφή στο Move & Pose',
    },
    service: {
      eyebrow: 'Η υπηρεσία',
      title: 'Τι καλύπτει',
      items: [
        {
          title: 'Live & Online Posing',
          body: 'Coaching live στην πλάτφορμα ή online μέσω video — όπου κι αν είστε, πριν και κατά τη διάρκεια της προετοιμασίας σας.',
        },
        {
          title: 'Posing & presentation',
          body: 'Στάσεις, transitions, stage presence και χορογραφία που αναδεικνύουν τη φυσική σας.',
        },
        {
          title: 'Προσωπικό πλάνο',
          body: 'Προσαρμογή στο division, το music και το level σας — από πρώτη φορά μέχρι πλάτφορμα.',
        },
        {
          title: 'Custom Bikinis',
          body: 'Σχεδιασμός και κατασκευή competition bikini προσαρμοσμένο στο σώμα και το division σας.',
        },
        {
          title: 'Makeup service',
          body: 'Stage makeup για διαγωνισμό — look που αναδεικνύει τη φυσική σας κάτω από τα φώτα της πλάτφορμας.',
        },
        {
          title: 'Custom Made Heels',
          body: 'Custom heels για Wellness, Bikini και σχετικές κατηγορίες — άνεση, σταθερότητα και σωστό ύψος για το stage.',
        },
      ],
      federations: {
        eyebrow: 'Federations',
        title: 'Προετοιμασία για τις μεγαλύτερες ομοσπονδίες',
        names: ['IFBB', 'NPC', 'WNBF', 'WABBA', 'FMG'],
      },
    },
    how: {
      title: 'Πώς λειτουργεί',
      steps: [
        { title: 'Κράτηση', body: 'Επιλέγετε πακέτο και κλείνετε διαθέσιμη ώρα στο ημερολόγιο.' },
        {
          title: 'Email & πληρωμή',
          body: 'Λαμβάνεις email με την τιμή και συνδέσμους πληρωμής (Stripe, PayPal ή Revolut).',
        },
        {
          title: 'Συνεδρία',
          body: 'Δουλεύετε live με τη Μαγδα — διόρθωση, repetition, recording tips.',
        },
      ],
    },
    pricing: {
      eyebrow: 'Πακέτα & τιμές',
      title: 'Διάλεξε το πρόγραμμα που σου ταιριάζει',
      body: 'Όλα τα πακέτα είναι για online posing coaching. Κλείνετε ώρα πρώτα και ολοκληρώνετε την πληρωμή από το email που θα λάβετε.',
      choose: 'Επιλογή {{package}}',
      prevPackage: 'Προηγούμενο πακέτο',
      nextPackage: 'Επόμενο πακέτο',
      packagesAria: 'Πακέτα posing',
      swipeHint: 'Σύρε για άλλο πακέτο · ',
      counter: '{{current}} από {{total}}',
      includedTitle: 'Όλα τα sessions περιλαμβάνουν',
      included: ['Personalised walks', 'T-walks ανά federation', 'Comparison rounds'],
      packages: [
        {
          label: 'Μία συνεδρία',
          name: '1 Posing Session',
          cta: 'Επιλογή συνεδρίας',
          backgroundImage: '/posing-bg-single.webp',
          features: ['Μία online posing συνεδρία.'],
        },
        {
          label: 'Μηνιαίο πακέτο',
          name: 'Sapphire',
          cta: 'Επιλογή Sapphire',
          backgroundImage: '/posing-bg-sapphire.webp',
          features: [
            '15 min launch call',
            "1:1 posing session 30' ανά δύο εβδομάδες",
            'Check-ins & feedback ανά δύο εβδομάδες',
          ],
        },
        {
          label: 'Προπληρωμή',
          name: 'Ruby',
          cta: 'Επιλογή Ruby',
          backgroundImage: '/posing-bg-ruby.webp',
          features: [
            '15 min launch call',
            "Εβδομαδιαία 1:1 posing sessions 30'",
            'Εβδομαδιαία check-ins & feedback',
          ],
        },
        {
          label: 'Προπληρωμή',
          name: 'Diamond',
          cta: 'Επιλογή Diamond',
          backgroundImage: '/posing-bg-diamond.webp',
          features: [
            '15 min launch call',
            "Εβδομαδιαία 1:1 posing sessions 40'",
            'Εβδομαδιαία check-ins & feedback',
            'Stage presence & heels technique',
          ],
        },
      ],
    },
    booking: {
      eyebrow: 'Κράτηση & πληρωμή',
      title: 'Κλείσε τη συνεδρία σου',
      body: 'Συνδέσου στον λογαριασμό σου, επίλεξε διαθέσιμη ώρα — η τιμή και οι σύνδεσμοι πληρωμής θα σταλούν στο email σου.',
      selectedLabel: 'Επιλεγμένο πακέτο',
      paymentSuccess: 'Η πληρωμή ολοκληρώθηκε. Σας ευχαριστούμε — θα λάβετε επιβεβαίωση από το Stripe.',
      sessionsIncluded: '{{count}} συνεδρίες',
      sessionsRemaining: 'Απομένουν {{remaining}} από {{total}}',
      sessionsRemainingShort: '{{remaining}} διαθέσιμες κρατήσεις',
      newPurchaseHint: 'Νέα αγορά — θα λάβεις email πληρωμής',
      paidPackageHint: 'Πληρωμένο πακέτο',
      paymentRequiredFirst: 'Ολοκλήρωσε την πληρωμή πριν κλείσεις επόμενη συνεδρία',
      sessionsExhausted: 'Έχεις κλείσει όλες τις συνεδρίες αυτού του πακέτου',
      confirmIncluded: 'Επιβεβαίωση κράτησης',
      stickyConfirm: 'Επιβεβαίωση κράτησης',
      selectedTimeLabel: 'Επιλεγμένη ώρα',
      stepConfirm: 'Βήμα 2: Επιβεβαίωση',
      successIncluded: 'Η συνεδρία επιβεβαιώθηκε.',
      bookWithPackage: 'Κλείσε με αυτό το πακέτο',
    },
    offers: {
      heroCta: 'Προσφορές',
      eyebrow: 'Προσφορά Ιουλίου',
      title: 'July offer',
      subtitle: '8 συνεδρίες 1:1 — Ruby ή Diamond. Ισχύει για κρατήσεις Ιουλίου.',
      badge: '-15%',
      rubyTitle: 'Ruby x8',
      diamondTitle: 'Diamond x8',
      sessionsLabel: '8 × 1:1 posing sessions',
      onlyJuly: 'Μόνο για κρατήσεις Ιουλίου',
      priceInEmail: 'Η τιμή θα σταλεί στο email επιβεβαίωσης',
      ctaRuby: 'Κλείσε Ruby x8',
      ctaDiamond: 'Κλείσε Diamond x8',
      modalDismiss: 'Όχι τώρα',
      modalCloseAria: 'Κλείσιμο προσφορών',
    },
    auth: {
      login: 'Σύνδεση',
      signup: 'Εγγραφή',
      logout: 'Αποσύνδεση',
      myAccount: 'Ο λογαριασμός μου',
      loginTitle: 'Σύνδεση',
      loginBody: 'Συνδέσου για να κλείσεις συνεδρία και να δεις τις κρατήσεις σου.',
      signupTitle: 'Δημιουργία λογαριασμού',
      signupBody: 'Φτιάξε λογαριασμό Move & Pose για κρατήσεις και πακέτα.',
      fullName: 'Ονοματεπώνυμο',
      password: 'Κωδικός',
      confirmPassword: 'Επιβεβαίωση κωδικού',
      passwordMismatch: 'Οι κωδικοί δεν ταιριάζουν.',
      showPassword: 'Εμφάνιση κωδικού',
      hidePassword: 'Απόκρυψη κωδικού',
      noAccount: 'Δεν έχεις λογαριασμό;',
      hasAccount: 'Έχεις ήδη λογαριασμό;',
      signingIn: 'Σύνδεση…',
      signingUp: 'Εγγραφή…',
      signupSuccessTitle: 'Έλεγξε το email σου',
      signupSuccessBody: 'Σου στείλαμε email επιβεβαίωσης. Μετά την ενεργοποίηση, συνδέσου για να κλείσεις συνεδρία.',
      errorGeneric: 'Κάτι πήγε στραβά. Δοκίμασε ξανά.',
      invalidCredentials:
        'Λάθος email ή κωδικός. Αν μόλις έκανες εγγραφή, πρώτα επιβεβαίωσε το email (έλεγξε και spam).',
      emailNotConfirmed: 'Πρέπει να επιβεβαιώσεις το email πριν τη σύνδεση. Έλεγξε inbox και spam.',
      rateLimit: 'Πολλά email από το Supabase. Δοκίμασε αργότερα ή δημιούργησε τον χρήστη χειροκίνητα στο dashboard.',
      notConfigured: 'Το σύστημα κρατήσεων ρυθμίζεται. Επικοινώνησε μαζί μας για κράτηση.',
      forgotPassword: 'Ξέχασες τον κωδικό;',
      forgotPasswordTitle: 'Επαναφορά κωδικού',
      forgotPasswordBody:
        'Βάλε το email του λογαριασμού σου. Αν υπάρχει εγγραφή, θα σου στείλουμε σύνδεσμο για νέο κωδικό.',
      sendResetLink: 'Αποστολή συνδέσμου',
      sendingResetLink: 'Αποστολή…',
      resetEmailSentTitle: 'Έλεγξε το email σου',
      resetEmailSentBody:
        'Αν υπάρχει λογαριασμός με αυτό το email, θα λάβεις σύνδεσμο επαναφοράς. Έλεγξε και spam.',
      resetPasswordTitle: 'Νέος κωδικός',
      resetPasswordBody: 'Διάλεξε νέο κωδικό (τουλάχιστον 8 χαρακτήρες).',
      setNewPassword: 'Αποθήκευση κωδικού',
      settingPassword: 'Αποθήκευση…',
      resetPasswordSuccess: 'Ο κωδικός άλλαξε. Μπορείς να συνδεθείς με τον νέο κωδικό.',
      resetLinkInvalid: 'Ο σύνδεσμος έληξε ή δεν είναι έγκυρος. Ζήτησε νέο από την επαναφορά κωδικού.',
      samePassword: 'Ο νέος κωδικός πρέπει να είναι διαφορετικός από τον προηγούμενο.',
      weakPassword: 'Ο κωδικός είναι πολύ ασθενής ή σύντομος.',
      backToLogin: 'Πίσω στη σύνδεση',
      continueWithGoogle: 'Συνέχεια με Google',
      orContinueWithEmail: 'ή με email',
      oauthRedirecting: 'Μετάβαση…',
    },
    account: {
      title: 'Ο λογαριασμός μου',
      loading: 'Φόρτωση…',
      profileTitle: 'Το προφίλ μου',
      profileBody: 'Συμπλήρωσε τα στοιχεία επικοινωνίας σου — χρησιμοποιούνται για κρατήσεις και επικοινωνία με τη coach.',
      fullName: 'Ονοματεπώνυμο',
      phone: 'Τηλέφωνο',
      division: 'Division / κατηγορία',
      divisionPlaceholder: 'π.χ. Bikini, Wellness, Figure',
      notes: 'Σημειώσεις',
      notesPlaceholder: 'Επίπεδο, ημερομηνία αγώνα, ότι χρειάζεται να ξέρουμε.',
      saveProfile: 'Αποθήκευση προφίλ',
      editProfile: 'Επεξεργασία',
      cancelEdit: 'Ακύρωση',
      savingProfile: 'Αποθήκευση…',
      profileSaved: 'Το προφίλ ενημερώθηκε.',
      avatarChange: 'Αλλαγή φωτογραφίας',
      avatarRemove: 'Αφαίρεση φωτογραφίας',
      avatarUploading: 'Ανέβασμα…',
      memberSince: 'Μέλος από {{date}}',
      backHome: 'Αρχική',
      quickStatsPackages: 'Ενεργά πακέτα',
      quickStatsSessionsRemaining: 'Διαθέσιμες συνεδρίες',
      quickStatsBookings: 'Επερχόμενες κρατήσεις',
      securityTitle: 'Ασφάλεια',
      password: 'Κωδικός',
      dangerZoneTitle: 'Προχωρημένα',
      dangerZoneExpand: 'Εμφάνιση',
      passwordTitle: 'Αλλαγή κωδικού',
      passwordBody: 'Βάλε τον τρέχοντα κωδικό και τον νέο (τουλάχιστον 8 χαρακτήρες).',
      currentPassword: 'Τρέχων κωδικός',
      newPassword: 'Νέος κωδικός',
      confirmPassword: 'Επιβεβαίωση νέου κωδικού',
      changePassword: 'Αλλαγή κωδικού',
      changingPassword: 'Αλλαγή…',
      passwordChanged: 'Ο κωδικός ενημερώθηκε.',
      passwordMismatch: 'Οι νέοι κωδικοί δεν ταιριάζουν.',
      deleteTitle: 'Διαγραφή λογαριασμού',
      deleteBody: 'Η διαγραφή είναι μόνιμη. Χάνεις πρόσβαση σε κρατήσεις και πακέτα.',
      deletePassword: 'Κωδικός για επιβεβαίωση',
      deleteConfirm: 'Διαγραφή λογαριασμού',
      deleting: 'Διαγραφή…',
      deleteSuccess: 'Ο λογαριασμός διαγράφηκε.',
      activePackages: 'Ενεργά πακέτα',
      noPackages: 'Δεν έχεις ενεργό πακέτο ακόμα.',
      remaining: '{{remaining}} απομένουν από {{total}} sessions',
      upcoming: 'Επερχόμενες κρατήσεις',
      noBookings: 'Δεν έχεις κρατήσεις.',
      bookNew: 'Κλείσε νέα συνεδρία',
      history: 'Ιστορικό',
      historyModalTitle: 'Ιστορικό κρατήσεων',
      showAllHistory: 'Δες όλα ({{count}})',
      avatarPreview: 'Προβολή φωτογραφίας',
      avatarPreviewClose: 'Κλείσιμο',
      pendingOrders: 'Εκκρεμείς παραγγελίες',
      pendingOrdersBody: 'Ολοκλήρωσε την πληρωμή ή ακύρωσε την παραγγελία.',
      cancelOrder: 'Ακύρωση παραγγελίας',
      cancelBooking: 'Ακύρωση κράτησης',
      addToCalendar: 'Πρόσθεσε στο ημερολόγιο',
      downloadingCalendar: 'Λήψη…',
      cancelOrderTitle: 'Ακύρωση παραγγελίας;',
      cancelOrderBody: 'Η κράτηση και το πακέτο θα ακυρωθούν. Η ώρα θα ξαναγίνει διαθέσιμη.',
      cancelBookingTitle: 'Ακύρωση κράτησης;',
      cancelBookingBody: 'Η συνεδρία θα ακυρωθεί και θα επιστρέψει 1 session στο πακέτο σου.',
      status: {
        pending_payment: 'Αναμονή πληρωμής',
        confirmed: 'Επιβεβαιωμένη',
        cancelled: 'Ακυρωμένη',
        completed: 'Ολοκληρωμένη',
      },
      errors: {
        server_config_error: 'Πρόβλημα ρύθμισης server (έλεγξε SUPABASE_SERVICE_ROLE_KEY στο Vercel).',
        invalid_password: 'Λάθος κωδικός.',
        admin_cannot_delete: 'Οι admin λογαριασμοί δεν διαγράφονται από εδώ.',
        active_bookings: 'Έχεις ενεργή κράτηση. Ακύρωσέ την πρώτα ή επικοινώνησε μαζί μας.',
        password_required: 'Συμπλήρωσε τον κωδικό σου.',
        password_too_short: 'Ο νέος κωδικός πρέπει να έχει τουλάχιστον 8 χαρακτήρες.',
        password_same_as_current: 'Ο νέος κωδικός πρέπει να είναι διαφορετικός από τον τρέχοντα.',
        password_change_failed: 'Η αλλαγή κωδικού απέτυχε. Δοκίμασε ξανά.',
        delete_failed: 'Η διαγραφή απέτυχε. Δοκίμασε ξανά.',
        avatar_invalid_type: 'Μόνο JPG, PNG ή WebP.',
        avatar_too_large: 'Μέγιστο μέγεθος 2 MB.',
        avatar_resize_failed: 'Αποτυχία επεξεργασίας εικόνας.',
        avatar_upload_failed: 'Το ανέβασμα απέτυχε. Δοκίμασε ξανά.',
        migration_avatar_required:
          'Για avatars τρέξε το supabase/migrations/003_profile_avatar.sql στο Supabase SQL Editor.',
        cannot_cancel: 'Δεν μπορείς να ακυρώσεις αυτή την κράτηση.',
        cancel_failed: 'Η ακύρωση απέτυχε. Δοκίμασε ξανά.',
        booking_not_found: 'Η κράτηση δεν βρέθηκε.',
        calendar_download_failed: 'Αποτυχία λήψης ημερολογίου. Δοκίμασε ξανά.',
        not_confirmed: 'Το ημερολόγιο είναι διαθέσιμο μόνο για επιβεβαιωμένες συνεδρίες.',
        calendar_unavailable: 'Δεν βρέθηκαν στοιχεία συνεδρίας για το ημερολόγιο.',
        calendar_failed: 'Αποτυχία λήψης ημερολογίου. Δοκίμασε ξανά.',
        unauthorized: 'Συνδέσου για να κατεβάσεις το ημερολόγιο.',
        method_not_allowed: 'Η λήψη ημερολογίου δεν είναι διαθέσιμη ακόμα. Δοκίμασε ξανά σε λίγο.',
      },
    },
    calendar: {
      title: 'Διαθέσιμες ώρες',
      loading: 'Φόρτωση διαθεσιμότητας…',
      noSlots: '—',
      loginPrompt: 'Για κράτηση χρειάζεται λογαριασμός.',
      confirm: 'Επιβεβαίωση κράτησης',
      booking: 'Κράτηση…',
      success: 'Η κράτηση καταχωρήθηκε. Έλεγξε το email σου για πληρωμή.',
      error: 'Η κράτηση απέτυχε. Δοκίμασε ξανά.',
      slotTaken: 'Η ώρα μόλις κλείστηκε. Επίλεξε άλλη.',
      loginRequired: 'Συνδέσου για να κλείσεις ώρα.',
      sessionExpired: 'Η σύνδεσή σου έληξε. Συνδέσου ξανά για να ολοκληρώσεις την κράτηση.',
      errors: {
        server_config_error:
          'Πρόβλημα ρύθμισης server. Έλεγξε SUPABASE_URL και SUPABASE_SERVICE_ROLE_KEY στο Vercel και κάνε redeploy.',
        missing_supabase_admin_config:
          'Λείπουν SUPABASE_URL ή SUPABASE_SERVICE_ROLE_KEY στο Vercel. Κάνε redeploy μετά την προσθήκη.',
        invalid_api_response: 'Μη έγκυρη απάντηση από το server. Δοκίμασε ξανά σε λίγο.',
        slots_fetch_failed: 'Αποτυχία φόρτωσης ωρών. Δοκίμασε ξανά.',
      },
    },
    admin: {
      title: 'Διαχείριση',
      subtitle: 'Κρατήσεις, μέλη και πληρωμές Move & Pose',
      eyebrow: 'Πάνελ διαχείρισης',
      backToPosing: '← Επιστροφή στο Move & Pose',
      refresh: 'Ανανέωση',
      forbidden: 'Δεν έχεις πρόσβαση admin.',
      openCalendar: 'Άνοιγμα ημερολογίου',
      reviewPayments: 'Έλεγχος εκκρεμών πληρωμών',
      actions: 'Ενέργειες',
      confirmDeleteMemberTitle: 'Διαγραφή μέλους;',
      confirmDeleteMemberBody: 'Ο λογαριασμός και τα σχετικά δεδομένα διαγράφονται οριστικά. Η ενέργεια δεν αναιρείται.',
      tabOverview: 'Επισκόπηση',
      tabCalendar: 'Ημερολόγιο',
      tabMembers: 'Μέλη',
      tabPayments: 'Πληρωμές',
      tabPrograms: 'Moove Προγράμματα',
      tabNutrition: 'Διατροφή',
      tabBookings: 'Κρατήσεις',
      accountSettings: 'Ρυθμίσεις λογαριασμού',
      statMembers: 'Μέλη',
      statPendingPayments: 'Εκκρεμείς πληρωμές',
      statActivePackages: 'Ενεργά πακέτα',
      statWeekBookings: 'Κρατήσεις εβδομάδας',
      pendingPaymentsTitle: 'Εκκρεμείς πληρωμές',
      viewAll: 'Όλες',
      noPendingPayments: 'Δεν υπάρχουν εκκρεμείς πληρωμές.',
      confirmPayment: 'Επιβεβαίωση πληρωμής',
      confirmingPayment: 'Επιβεβαίωση…',
      confirmPaymentTitle: 'Επιβεβαίωση πληρωμής;',
      confirmPaymentBody: 'Η κράτηση θα επιβεβαιωθεί χειροκίνητα (π.χ. μετρητά ή τραπεζική κατάθεση).',
      allBookings: 'Όλες οι κρατήσεις',
      allBookingsBody: 'Πλήρης λίστα κρατήσεων με φίλτρο κατάστασης.',
      filterAll: 'Όλες',
      noBookings: 'Δεν υπάρχουν κρατήσεις.',
      memberActivePackages: '{{count}} ενεργά πακέτα',
      memberPendingPackages: '{{count}} εκκρεμεί πληρωμή',
      memberNoPackages: 'Χωρίς πακέτα',
      recentBookings: 'Πρόσφατες κρατήσεις',
      packageStatus: {
        pending_payment: 'Αναμονή πληρωμής',
        active: 'Ενεργό',
        expired: 'Έληξε',
        cancelled: 'Ακυρωμένο',
      },
      addSlot: 'Προσθήκη διαθέσιμης ώρας',
      weekTitle: 'Εβδομαδιαίο πρόγραμμα',
      thisWeek: 'Αυτή την εβδομάδα',
      previousWeek: 'Προηγούμενη εβδομάδα',
      nextWeek: 'Επόμενη εβδομάδα',
      date: 'Ημερομηνία',
      time: 'Ώρα',
      duration: 'Διάρκεια',
      add: 'Προσθήκη',
      slots: 'Διαθέσιμες ώρες',
      noSlots: 'Δεν υπάρχουν ώρες σε αυτή την περίοδο.',
      noDaySlots: 'Καμία ώρα',
      noWeekBookings: 'Δεν υπάρχουν κρατήσεις αυτή την εβδομάδα.',
      free: 'Ελεύθερη',
      delete: 'Διαγραφή',
      bookings: 'Κρατήσεις',
      legendEmpty: 'Κενό',
      legendOpen: 'Διαθέσιμο',
      legendBooked: 'Κρατημένο',
      openDay: 'Άνοιγμα',
      clearDay: 'Καθάρισμα',
      opening: 'Ενημέρωση…',
      durationHint: 'Οι νέες ώρες δημιουργούνται με διάρκεια {{minutes}} λεπτά.',
      slotsCreated: 'Προστέθηκαν {{count}} ώρες.',
      slotsDeleted: 'Διαγράφηκαν {{count}} ώρες.',
      slotBooked: 'Κράτηση',
      calendarSettings: 'Ρυθμίσεις ημερολογίου',
      dayTemplate: 'Πρότυπο «Άνοιγμα ημέρας»',
      dayTemplateHint: 'Ώρες που δημιουργούνται με το κουμπί «Άνοιγμα» στη στήλη της ημέρας.',
      addTime: 'Προσθήκη ώρας',
      addTimeShort: '+ Ώρα',
      removeTime: 'Αφαίρεση',
      noTemplateTimes: 'Δεν υπάρχουν ώρες στο πρότυπο.',
      gridStart: 'Πλέγμα από',
      gridEnd: 'Πλέγμα έως',
      gridEnvelopeStart: 'Εμφάνιση πλέγματος από',
      gridEnvelopeEnd: 'Εμφάνιση πλέγματος έως',
      gridEnvelopeHint:
        'Ορίζει ποιες ώρες εμφανίζονται στο grid. Η κατάσταση κάθε κελιού (κενό / διαθέσιμο / ανενεργό) ορίζεται με δεξί κλικ ή παρατεταμένο πάτημα.',
      openSettings: 'Ρυθμίσεις',
      closeSettings: 'Κλείσιμο',
      cancelSettings: 'Ακύρωση',
      cellMenuHint: 'Δεξί κλικ ή παρατεταμένο πάτημα σε ώρα για αλλαγή κατάστασης.',
      cellMenuHintMobile: 'Παρατεταμένο πάτημα σε ώρα για αλλαγή κατάστασης.',
      weekdayActiveFrom: 'Ενεργές από',
      weekdayActiveUntil: 'Ενεργές έως',
      activeHoursSummary: '{{weekday}}: ενεργές {{from}}–{{until}}',
      gridStep: 'Βήμα πλέγματος',
      defaultDuration: 'Προεπιλεγμένη διάρκεια',
      saveSettings: 'Αποθήκευση',
      savingSettings: 'Αποθήκευση…',
      settingsSaved: 'Οι ρυθμίσεις ημερολογίου αποθηκεύτηκαν.',
      weekdayActiveHours: 'Πρόγραμμα ανά ημέρα',
      weekdayMon: 'Δευ',
      weekdayTue: 'Τρι',
      weekdayWed: 'Τετ',
      weekdayThu: 'Πέμ',
      weekdayFri: 'Παρ',
      weekdaySat: 'Σάβ',
      weekdaySun: 'Κυρ',
      legendInactive: 'Ανενεργό',
      membersTitle: 'Μέλη',
      membersBody: 'Όλοι οι εγγεγραμμένοι χρήστες Move & Pose.',
      membersCount: '{{count}} μέλη',
      noMembers: 'Δεν υπάρχουν μέλη ακόμα.',
      memberSince: 'Εγγραφή',
      you: 'εσύ',
      deleteMember: 'Διαγραφή',
      customPricesTitle: 'Ειδικές τιμές',
      customPricesBody: 'Οι τιμές ισχύουν μόνο για αυτόν τον πελάτη και εμφανίζονται στο email κράτησης.',
      catalogPrice: 'Κανονική: {{price}}',
      customPriceActive: 'Ειδική: {{price}}',
      savePrice: 'Αποθήκευση',
      removeOverride: 'Επαναφορά',
      customPriceInvalid: 'Μη έγκυρη τιμή.',
      customPriceSaveFailed: 'Αποτυχία αποθήκευσης τιμής.',
      confirmDeleteMember: 'Ναι, διαγραφή',
      cancelDeleteMember: 'Άκυρο',
      deletingMember: 'Διαγραφή…',
      memberErrors: {
        cannot_delete_self: 'Δεν μπορείς να διαγράψεις τον δικό σου λογαριασμό από εδώ.',
        cannot_delete_admin: 'Δεν διαγράφονται admin λογαριασμοί.',
        member_not_found: 'Ο χρήστης δεν βρέθηκε.',
        delete_member_failed: 'Η διαγραφή απέτυχε.',
        price_save_failed: 'Αποτυχία αποθήκευσης τιμής.',
        price_delete_failed: 'Αποτυχία διαγραφής ειδικής τιμής.',
        invalid_price: 'Μη έγκυρη τιμή.',
      },
      adminErrors: {
        load_failed: 'Αποτυχία φόρτωσης δεδομένων.',
        slots_failed: 'Αποτυχία φόρτωσης ημερολογίου.',
        toggle_failed: 'Αποτυχία ενημέρωσης ώρας.',
        admin_slot_update_failed: 'Αποτυχία ενημέρωσης ώρας.',
        preset_failed: 'Αποτυχία ανοίγματος ημέρας.',
        clear_failed: 'Αποτυχία καθαρισμού ημέρας.',
        payment_confirm_failed: 'Η επιβεβαίωση πληρωμής απέτυχε.',
        missing_email_config:
          'Δεν έχει ρυθμιστεί αποστολή email. Τοπικά: βάλε το RESEND_API_KEY από Vercel (Preview/Production) στο .env.local. Η πληρωμή δεν επιβεβαιώθηκε.',
        payment_confirmed_email_failed:
          'Η πληρωμή επιβεβαιώθηκε αλλά το email απέτυχε. Χρησιμοποίησε «Επανάληψη αποστολής link».',
        overview_fetch_failed: 'Αποτυχία φόρτωσης επισκόπησης.',
        payments_fetch_failed: 'Αποτυχία φόρτωσης πληρωμών.',
        bookings_fetch_failed: 'Αποτυχία φόρτωσης κρατήσεων.',
        members_fetch_failed: 'Αποτυχία φόρτωσης μελών.',
        settings_fetch_failed: 'Αποτυχία φόρτωσης ρυθμίσεων ημερολογίου.',
        settings_save_failed: 'Αποτυχία αποθήκευσης ρυθμίσεων ημερολογίου.',
        invalid_time: 'Μη έγκυρη μορφή ώρας.',
        time_exists: 'Υπάρχει ήδη ώρα σε αυτή την ώρα.',
        invalid_weekday_templates: 'Μη έγκυρες ρυθμίσεις προγράμματος ανά ημέρα.',
        outside_active_hours: 'Η ώρα είναι εκτός των ενεργών ωρών αυτής της ημέρας.',
        migration_required:
          'Απαιτείται migration βάσης. Τρέξε το supabase/scripts/apply_posing_calendar_settings.sql στο Supabase SQL Editor.',
      },
    },
    faq: {
      title: 'Συχνές ερωτήσεις',
      items: [
        {
          q: 'Έχει σχέση με τα μαθήματα του studio;',
          a: 'Όχι. Το Move & Pose είναι ξεχωριστή υπηρεσία online coaching για bodybuilding shows. Το πρόγραμμα του γυμναστηρίου κλείνεται ξεχωριστά.',
        },
        {
          q: 'Τι χρειάζομαι;',
          a: 'Σταθερό internet, κάμερα (κινητό ή laptop), χώρο να κινηθείτε και optional posing trunks / competition wear.',
        },
        {
          q: 'Σε ποια divisions;',
          a: "Bikini, Wellness, Figure, Women's Physique και σχετικές κατηγορίες — ρωτήστε αν δεν είστε σίγουροι.",
        },
        {
          q: 'Γλώσσα;',
          a: 'Ελληνικά και αγγλικά.',
        },
        {
          q: 'Πώς γίνεται η πληρωμή;',
          a: 'Κλείνεις ώρα από το ημερολόγιο (απαιτείται λογαριασμός) και λαμβάνεις email με την τιμή και συνδέσμους πληρωμής (Stripe, PayPal ή Revolut). Η συνεδρία επιβεβαιώνεται μετά την πληρωμή.',
        },
      ],
    },
    cal: {
      pending:
        'Το online ημερολόγιο κρατήσεων ρυθμίζεται. Μέχρι τότε, στείλτε αίτημα ραντεβού και θα επικοινωνήσουμε για επιβεβαίωση:',
      sent: 'Το email αιτήματος άνοιξε στη συσκευή σας. Στείλτε το για να ολοκληρωθεί το αίτημα.',
      emailSubject: 'Posing coaching — κράτηση',
      emailIntro: 'Θα ήθελα να κλείσω ραντεβού για Move & Pose.',
      name: 'Ονοματεπώνυμο',
      namePlaceholder: 'π.χ. Μαρία Παπαδοπούλου',
      phone: 'Τηλέφωνο',
      phonePlaceholder: 'π.χ. +30 69...',
      emailPlaceholder: 'το email σου',
      preferredDate: 'Προτιμώμενη ημέρα / ώρα',
      sessionType: 'Τύπος συνεδρίας',
      message: 'Σημειώσεις',
      messagePlaceholder: 'Division, επίπεδο, ημερομηνία αγώνα ή ό,τι χρειάζεται να ξέρουμε.',
      submit: 'Αίτημα ραντεβού',
      options: {
        online: 'Online posing session',
        live: 'Live posing session',
        bikini: 'Custom bikini',
        makeup: 'Makeup service',
        other: 'Άλλη υπηρεσία',
      },
    },
  },
  poseBubble: {
    tagline: 'Online posing coaching',
    homeBody:
      'Ξεχωριστή υπηρεσία για bodybuilding shows — live coaching, στάσεις και stage presence. Δεν σχετίζεται με τα μαθήματα του studio.',
    studioTitle: '{{name}} Studio',
    studioSubtitle: 'Pilates & Reformer · Βόλος',
  },
  cookies: {
    banner: {
      ariaLabel: 'Ειδοποίηση cookies',
      title: 'Χρησιμοποιούμε cookies',
      bodyBefore:
        'Χρησιμοποιούμε απαραίτητα cookies για τη λειτουργία του site (π.χ. γλώσσα) και, με τη συγκατάθεσή σας, cookies ανάλυσης. Δείτε την',
      bodyMiddle: 'και την',
      bodyAfter: '.',
      policyLink: 'πολιτική cookies',
      privacyLink: 'πολιτική απορρήτου',
      acceptAll: 'Αποδοχή όλων',
      rejectOptional: 'Απόρριψη προαιρετικών',
      customize: 'Ρυθμίσεις',
    },
    preferences: {
      title: 'Ρυθμίσεις cookies',
      description:
        'Επιλέξτε ποια cookies επιτρέπετε. Τα απαραίτητα είναι πάντα ενεργά γιατί χρειάζονται για τη βασική λειτουργία του site.',
      save: 'Αποθήκευση επιλογών',
      essential: {
        title: 'Απαραίτητα',
        description:
          'Αποθηκεύουν προτιμήσεις όπως η γλώσσα εμφάνισης. Δεν μπορούν να απενεργοποιηθούν.',
        alwaysOn: 'Πάντα ενεργά',
      },
      analytics: {
        title: 'Ανάλυση επισκεψιμότητας',
        description:
          'Βοηθούν να κατανοούμε πώς χρησιμοποιείται το site (ανώνυμα στατιστικά μέσω Vercel Analytics).',
      },
    },
    footer: {
      cookiePolicy: 'Πολιτική cookies',
      manageCookies: 'Ρυθμίσεις cookies',
    },
    page: {
      eyebrow: 'Νομικά',
      title: 'Πολιτική cookies',
      description:
        'Πώς χρησιμοποιεί το {{name}} cookies και παρόμοια τεχνολογία στο site του studio.',
      lastUpdated: 'Τελευταία ενημέρωση: Ιούνιος 2026',
      backHome: 'Επιστροφή στην αρχική',
      sections: [
        {
          title: 'Τι είναι τα cookies',
          body: 'Τα cookies είναι μικρά αρχεία που αποθηκεύονται στη συσκευή σας όταν επισκέπτεστε έναν ιστότοπο. Μας βοηθούν να θυμόμαστε προτιμήσεις και, με τη συγκατάθεσή σας, να κατανοούμε πώς χρησιμοποιείται το site.',
        },
        {
          title: 'Ποια cookies χρησιμοποιούμε',
          body: 'Απαραίτητα: αποθήκευση γλώσσας (localStorage). Προαιρετικά — μόνο με συγκατάθεση: ανώνυμα στατιστικά επισκεψιμότητας μέσω Vercel Analytics, χωρίς διαφημιστική παρακολούθηση.',
        },
        {
          title: 'Νομική βάση (GDPR)',
          body: 'Τα απαραίτητα cookies βασίζονται στο έννομο συμφέρον για τη λειτουργία του site. Τα cookies ανάλυσης ενεργοποιούνται μόνο με τη ρητή συγκατάθεσή σας, την οποία μπορείτε να αλλάξετε ανά πάσα στιγμή.',
        },
        {
          title: 'Πώς αλλάζετε τις επιλογές σας',
          body: 'Μπορείτε να αποδεχτείτε, να απορρίψετε ή να προσαρμόσετε τα προαιρετικά cookies από το banner κατά την πρώτη επίσκεψη, ή οποτεδήποτε από το link «Ρυθμίσεις cookies» στο footer.',
        },
        {
          title: 'Διάρκεια αποθήκευσης',
          body: 'Η επιλογή συγκατάθεσής σας αποθηκεύεται τοπικά στη συσκευή σας μέχρι να την αλλάξετε ή να διαγράψετε τα δεδομένα του browser. Η προτίμηση γλώσσας διατηρείται για διευκόλυνση στις επόμενες επισκέψεις.',
        },
      ],
    },
  },
  privacy: {
    footer: {
      privacyPolicy: 'Πολιτική απορρήτου',
    },
    page: {
      eyebrow: 'Νομικά',
      title: 'Πολιτική απορρήτου',
      description:
        'Πώς το {{name}} συλλέγει, χρησιμοποιεί και προστατεύει τα προσωπικά σας δεδομένα.',
      lastUpdated: 'Τελευταία ενημέρωση: Ιούνιος 2026',
      backHome: 'Επιστροφή στην αρχική',
      viewCookies: 'Πολιτική cookies',
      sections: [
        {
          title: 'Υπεύθυνος επεξεργασίας',
          body: 'Υπεύθυνη επεξεργασίας είναι η {{owner}} ({{name}}), {{address}}. Για θέματα απορρήτου: {{email}}.',
        },
        {
          title: 'Ποια δεδομένα συλλέγουμε',
          body: 'Μπορεί να επεξεργαζόμαστε: στοιχεία επικοινωνίας (όνομα, email, μήνυμα) από τη φόρμα επικοινωνίας· email για αγορά έτοιμων προγραμμάτων γυμναστικής· τεχνικά δεδομένα επίσκεψης (IP, browser, συσκευή) μέσω hosting/analytics· προτιμήσεις site (γλώσσα, cookies). Κρατήσεις μαθημάτων γίνονται σε εξωτερική πλατφόρμα (members.moovefitness.gr). Κρατήσεις Move & Pose και αγορές προγραμμάτων Moove γίνονται μέσω του site με online πληρωμές (Stripe, PayPal, Revolut), με δικές τους πολιτικές απορρήτου όπου ισχύουν.',
        },
        {
          title: 'Σκοπός επεξεργασίας',
          body: 'Απαντάμε σε αιτήματα επικοινωνίας, λειτουργούμε και βελτιώνουμε το site, εμφανίζουμε δημόσιες κριτικές Google και — με συγκατάθεση — αναλύουμε ανώνυμα την επισκεψιμότητα.',
        },
        {
          title: 'Νομική βάση (GDPR)',
          body: 'Επικοινωνία: έννομο συμφέρον ή προσυμβατικά μέτρα. Λειτουργία site: έννομο συμφέρον. Analytics: συγκατάθεση. Μπορείτε να ανακαλέσετε τη συγκατάθεση ανά πάσα στιγμή χωρίς να επηρεαστεί η νομιμότητα προηγούμενης επεξεργασίας.',
        },
        {
          title: 'Αποδέκτες και τρίτα μέρη',
          body: 'Χρησιμοποιούμε Vercel (hosting, προαιρετικά analytics), Google Places API (δημόσιες κριτικές), Stripe, PayPal και Revolut (online πληρωμές προγραμμάτων και Move & Pose) και εξωτερικές πλατφόρμες κρατήσεων. Οι πάροχοι επεξεργάζονται δεδομένα ως εκτελούντες την επεξεργασία ή ανεξάρτητοι υπεύθυνοι, σύμφωνα με τις δικές τους πολιτικές.',
        },
        {
          title: 'Διάρκεια διατήρησης',
          body: 'Μηνύματα επικοινωνίας: όσο χρειάζεται για απάντηση και τυχόν νομικές υποχρεώσεις. Logs hosting/analytics: σύμφωνα με τους παρόχους. Προτιμήσεις cookies/γλώσσας: μέχρι να τις αλλάξετε ή διαγράψετε τα δεδομένα browser.',
        },
        {
          title: 'Τα δικαιώματά σας',
          body: 'Έχετε δικαίωμα πρόσβασης, διόρθωσης, διαγραφής, περιορισμού, φορητότητας και εναντίωσης, καθώς και υποβολής καταγγελίας στην Αρχή Προστασίας Δεδομένων Προσωπικού Χαρακτήρα (www.dpa.gr).',
        },
        {
          title: 'Cookies',
          body: 'Για cookies και παρόμοια τεχνολογία, δείτε την ξεχωριστή πολιτική cookies και τις ρυθμίσεις συγκατάθεσης στο site.',
        },
      ],
    },
  },
  termsOfUse: {
    footer: {
      termsOfUse: 'Όροι χρήσης',
    },
    page: {
      eyebrow: 'Νομικά',
      title: 'Όροι χρήσης ιστότοπου',
      description: 'Κανόνες χρήσης του site του {{name}}.',
      lastUpdated: 'Τελευταία ενημέρωση: Ιούνιος 2026',
      backHome: 'Επιστροφή στην αρχική',
      sections: [
        {
          title: 'Αποδοχή όρων',
          body: 'Με την περιήγηση στο site αποδέχεστε τους παρόντες όρους. Αν δεν συμφωνείτε, παρακαλούμε μην χρησιμοποιείτε τον ιστότοπο.',
        },
        {
          title: 'Σκοπός του site',
          body: 'Το site παρέχει πληροφορίες για το {{name}}, τα μαθήματα Pilates & Reformer, την επικοινωνία και τη δυνατότητα κράτησης μέσω εξωτερικής πλατφόρμας. Δεν αποτελεί ιατρική συμβουλή.',
        },
        {
          title: 'Πνευματική ιδιοκτησία',
          body: 'Κείμενα, λογότυπα, φωτογραφίες και σχεδιασμός ανήκουν στο {{name}} ή στους δικαιούχους. Απαγορεύεται η αντιγραφή, αναπαραγωγή ή εμπορική χρήση χωρίς έγγραφη άδεια.',
        },
        {
          title: 'Εξωτερικοί σύνδεσμοι',
          body: 'Συνδέσμοι προς τρίτους (π.χ. κρατήσεις, πληρωμές μέσω Stripe, social media, Google Maps) οδηγούν σε εξωτερικούς ιστότοπους με δικές τους πολιτικές. Δεν ευθυνόμαστε για το περιεχόμενό τους.',
        },
        {
          title: 'Περιορισμός ευθύνης',
          body: 'Προσπαθούμε το site να είναι ακριβές και διαθέσιμο, αλλά δεν εγγυόμαστε αδιάλειπτη λειτουργία ή πλήρη ακρίβεια πληροφοριών. Η χρήση γίνεται με δική σας ευθύνη.',
        },
        {
          title: 'Τροποποιήσεις',
          body: 'Μπορούμε να ενημερώνουμε τους όρους. Η συνέχιση χρήσης μετά από αλλαγές συνιστά αποδοχή των νέων όρων.',
        },
        {
          title: 'Εφαρμοστέο δίκαιο',
          body: 'Οι όροι διέπονται από το ελληνικό δίκαιο. Αρμόδια δικαιοδοσία: δικαστήρια Βόλου, εφόσον επιτρέπεται από το νόμο.',
        },
        {
          title: 'Επικοινωνία',
          body: 'Για ερωτήσεις σχετικά με τους όρους χρήσης: {{email}}.',
        },
      ],
    },
  },
  nutrition: {
    eyebrow: 'Προσωποποιημένη διατροφή',
    title: 'Διατροφικό πλάνο',
    description:
      'Συμπλήρωσε το ερωτηματολόγιο και λάβε προσωποποιημένο διατροφικό πλάνο σε PDF στο email σου — αμέσως μετά την πληρωμή.',
    priceHint: 'Τιμή: 25€ · Πληρωμή μέσω Stripe, PayPal ή Revolut.',
    questionnaire: {
      steps: {
        body: 'Στοιχεία',
        activity: 'Δραστηριότητα',
        preferences: 'Προτιμήσεις',
        restrictions: 'Περιορισμοί',
      },
      age: 'Ηλικία',
      gender: 'Φύλο',
      genderFemale: 'Γυναίκα',
      genderMale: 'Άνδρας',
      height: 'Ύψος (cm)',
      weight: 'Βάρος (kg)',
      activity: 'Επίπεδο δραστηριότητας',
      activities: {
        sedentary: 'Καθιστική ζωή',
        light: 'Ελαφριά (1-2x/εβδομάδα)',
        moderate: 'Μέτρια (3-4x/εβδομάδα)',
        active: 'Ενεργή (5-6x/εβδομάδα)',
        very_active: 'Πολύ ενεργή (καθημερινά)',
      },
      goal: 'Στόχος',
      goals: {
        cut: 'Αδυνάτισμα',
        maintain: 'Διατήρηση',
        bulk: 'Μυϊκή μάζα',
      },
      dietType: 'Διατροφική προτίμηση',
      diets: {
        omnivore: 'Παμφαγία',
        vegetarian: 'Χορτοφαγία',
        vegan: 'Vegan',
      },
      mealsPerDay: 'Γεύματα ανά ημέρα',
      cookingLevel: 'Μαγείρεμα',
      cooking: {
        minimal: 'Ελάχιστο',
        moderate: 'Μέτριο',
        advanced: 'Προχωρημένο',
      },
      allergies: 'Αλλεργίες / δυσανεξίες',
      allergyLabels: {
        gluten: 'Γλουτένη',
        dairy: 'Γαλακτοκομικά',
        nuts: 'Ξηροί καρποί',
        eggs: 'Αυγά',
        soy: 'Σόγια',
        shellfish: 'Οστρακοειδή',
        fish: 'Ψάρι',
      },
      dislikedFoods: 'Τρόφιμα που δεν σου αρέσουν (προαιρετικό)',
      dislikedFoodsPlaceholder: 'π.χ. μπρόκολο, κουνουπίδι',
      name: 'Όνομα (προαιρετικό, για το PDF)',
      namePlaceholder: 'Το όνομά σου',
      email: 'Email',
      emailPlaceholder: 'το-email@example.com',
      allergyWarning:
        'Για σοβαρές αλλεργίες ή ιατρικά θέματα, συμβουλέψου γιατρό ή διαιτολόγο.',
      privacyPrefix: 'Αποδέχομαι την',
      privacyLink: 'πολιτική απορρήτου',
      disclaimer: 'Το πλάνο δεν αποτελεί ιατρική συμβουλή.',
      back: 'Πίσω',
      next: 'Επόμενο',
      submit: 'Συνέχεια στην πληρωμή',
      errors: {
        invalid_age: 'Ηλικία 16-80 ετών.',
        invalid_height: 'Ύψος 140-220 cm.',
        invalid_weight: 'Βάρος 40-200 kg.',
        invalid_email: 'Μη έγκυρο email.',
        privacy_required: 'Απαιτείται αποδοχή πολιτικής απορρήτου.',
      },
    },
    order: {
      cta: 'Αγορά διατροφής',
      submitting: 'Αποστολή…',
      successTitle: 'Έλεγξε το email',
      successBody: 'Στείλαμε email με συνδέσμους πληρωμής. Μετά την πληρωμή θα λάβεις το PDF.',
      refLabel: 'Κωδικός παραγγελίας',
      errors: {
        order_failed: 'Η παραγγελία απέτυχε. Δοκίμασε ξανά.',
        rate_limit: 'Πολλές παραγγελίες — δοκίμασε αργότερα.',
        invalid_email: 'Μη έγκυρο email.',
        privacy_required: 'Απαιτείται αποδοχή πολιτικής απορρήτου.',
      },
    },
    paymentSuccess: {
      title: 'Η πληρωμή ολοκληρώθηκε',
      body: 'Επεξεργαζόμαστε την παραγγελία σου…',
      generating: 'Δημιουργείται το πλάνο — θα το λάβεις στο email σύντομα.',
      sent: 'Το διατροφικό πλάνο στάλθηκε στο email σου.',
    },
    admin: {
      pendingTitle: 'Εκκρεμείς πληρωμές διατροφής',
      noPending: 'Δεν υπάρχουν εκκρεμείς πληρωμές διατροφής.',
      recentPaidTitle: 'Πρόσφατες παραγγελίες διατροφής',
      noPaid: 'Δεν υπάρχουν πρόσφατες παραγγελίες.',
      confirmBody: 'Επιβεβαίωση πληρωμής (PayPal/Revolut) — θα δημιουργηθεί και σταλεί PDF.',
      resendPlan: 'Επανάληψη PDF',
      resending: 'Αποστολή…',
      previewPdf: 'Προβολή PDF',
      typeStandalone: 'Standalone',
      typeAddon: 'Add-on πρόγραμμα',
      statusFailed: 'Αποτυχία δημιουργίας',
      viewResponses: 'Απαντήσεις ερωτηματολογίου',
    },
  },
  serviceTerms: {
    footer: {
      serviceTerms: 'Όροι υπηρεσιών',
    },
    page: {
      eyebrow: 'Νομικά',
      title: 'Όροι παροχής υπηρεσιών',
      description:
        'Όροι συμμετοχής στα μαθήματα και τις υπηρεσίες του {{name}} · {{address}}.',
      lastUpdated: 'Τελευταία ενημέρωση: Ιούνιος 2026',
      backHome: 'Επιστροφή στην αρχική',
      sections: [
        {
          title: 'Πεδίο εφαρμογής',
          body: 'Οι όροι αφορούν μαθήματα Pilates, Reformer και λοιπές υπηρεσίες του studio {{name}}. Το Move & Pose (online posing coaching) είναι ξεχωριστή υπηρεσία με δικούς της όρους.',
        },
        {
          title: 'Κρατήσεις',
          body: 'Οι κρατήσεις μαθημάτων γίνονται μέσω της online πλατφόρμας {{bookingUrl}}. Ισχύουν οι κανόνες, οι τιμές και οι πολιτικές της πλατφόρμας, όπως ισχύουν κάθε φορά.',
        },
        {
          title: 'Έτοιμα προγράμματα (digital)',
          body: 'Η αγορά έτοιμων προγραμμάτων γυμναστικής γίνεται online μέσω του site. Μετά την πληρωμή (Stripe, PayPal ή Revolut) λαμβάνετε email με link πρόσβασης σε προσωπική σελίδα (ασκησιολόγιο, videos). Το περιεχόμενο είναι για προσωπική χρήση· δεν επιτρέπεται αναδιανομή ή δημόσια κοινοποίηση.',
        },
        {
          title: 'Ακυρώσεις & απουσίες',
          body: 'Παρακαλούμε ακυρώνετε εγκαίρως σύμφωνα με την πολιτική του studio/πλατφόρμας κρατήσεων. Σε περίπτωση καθυστερημένης ακύρωσης ή no-show, μπορεί να εφαρμοστεί χρέωση ή απώλεια μαθήματος, όπως ορίζεται εκάστοτε.',
        },
        {
          title: 'Υγεία & ασφάλεια',
          body: 'Η συμμετοχή σε μαθήματα γυμναστικής απαιτεί καλή φυσική κατάσταση. Ενημερώστε την εκπαιδεύτρια για τραυματισμούς, εγκυμοσύνα ή ιατρικά θέματα. Συμβουλευτείτε γιατρό πριν ξεκινήσετε πρόγραμμα άσκησης. Συμμετέχετε με δική σας ευθύνη.',
        },
        {
          title: 'Κανόνες studio',
          body: 'Τηρείτε τις οδηγίες της {{owner}}, φτάνετε εγκαίρως, φοράτε κατάλληλη αθλητική ενδυμασία και σέβεστε τους υπόλοιπους ασκούμενους. Διατηρούμε το δικαίωμα άρνησης εισόδου σε περίπτωση ακατάλληλης συμπεριφοράς.',
        },
        {
          title: 'Ανήλικοι',
          body: 'Η συμμετοχή ανηλίκων επιτρέπεται μόνο με γονική/κηδεμονική συγκατάθεση και κατά την κρίση του studio, με σχετική ιατρική ενημέρωση όπου απαιτείται.',
        },
        {
          title: 'Φωτογραφίες & social media',
          body: 'Ενδέχεται να ληφθούν φωτογραφίες/βίντεο στο studio για προώθηση στα social media. Η δημοσίευση γίνεται μόνο με τη συγκατάθεσή σας. Μπορείτε να ζητήσετε ανά πάσα στιγμή να μην χρησιμοποιηθεί υλικό με την εικόνα σας.',
        },
        {
          title: 'Τιμές & τροποποιήσεις',
          body: 'Οι τιμές και το πρόγραμμα μαθημάτων μπορεί να αλλάζουν. Οι ισχύουσες τιμές και όροι ανακοινώνονται μέσω της πλατφόρμας κρατήσεων ή/και στο studio.',
        },
        {
          title: 'Περιορισμός ευθύνης',
          body: 'Το studio δεν ευθύνεται για τραυματισμούς που προκύπτουν από μη τήρηση οδηγιών, απόκρυψη ιατρικών πληροφοριών ή συμμετοχή κατά παράβαση των όρων, εντός των ορίων που επιτρέπει ο νόμος.',
        },
        {
          title: 'Επικοινωνία',
          body: 'Για θέματα υπηρεσιών studio: {{email}} · {{address}}.',
        },
      ],
    },
  },
} as const
