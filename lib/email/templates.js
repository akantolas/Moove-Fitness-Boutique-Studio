...
export function formatAmountEUR(amountEur, locale = 'el') {
...
function buildMooveProgramPaymentAlternativesBlock({
  ...
  const notes = []
  if (paypalLink && purchaseRef) {
    notes.push(
      isEl
        ? `Για πληρωμή με PayPal, στην περιγραφή γράψε τον κωδικό παραγγελίας ${purchaseRef}.`
        : `For PayPal, include order ref ${purchaseRef} in the payment description.`,
    )
  }
  if (revolutLink && amountLabel && purchaseRef) {
    notes.push(
      isEl
        ? `Για πληρωμή με Revolut, συμπλήρωσε ποσό ${amountLabel} και στο πεδίο «Σημείωση» γράψε τον κωδικό παραγγελίας ${purchaseRef}.`
        : `For Revolut, enter ${amountLabel} and add order ref ${purchaseRef} in the Note field.`,
    )
  }
  ...
  const bodyHtml = `
    <div style="margin:20px 0 0;padding:16px;background:${colors.outerBg};border:1px solid ${colors.cardBorder};border-radius:12px;">
      ...
      ${notes.map((note) => `<p style="margin:0 0 8px;font-family:system-ui,sans-serif;font-size:13px;line-height:1.5;color:${colors.muted};">${escapeHtml(note)}</p>`).join('')}
      <div>${links.join('')}</div>
    </div>`

  const bodyTextParts = [...notes]
...
export function buildMooveProgramPaymentEmail({
  ...
  const isEl = locale === 'el'
  const amountLabel = formatAmountEUR(amountEur, locale)
...
export function buildMooveProgramAccessEmail({ programName, accessUrl, locale = 'el' }) {
  ...
  return mooveTemplate({
    ...
    greeting: isEl ? 'Γεια σου,' : 'Hi,',
    intro: isEl
      ? 'Η πληρωμή ολοκληρώθηκε. Κάνε κλικ στο κουμπί, διάλεξε προπόνηση και δες όλες τις ασκήσεις και τα videos του προγράμματός σου.'
      : 'Payment completed. Click the button, choose a workout, and view every exercise and video in your program.',
