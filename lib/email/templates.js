...
function buildPaymentAlternativesBlock({
  ...
  const notes = []
  if (paypalLink && bookingRef) {
    notes.push(
      isEl
        ? `Για πληρωμή με PayPal, στην περιγραφή γράψε τον κωδικό κράτησης ${bookingRef}.`
        : `For PayPal, include booking ref ${bookingRef} in the payment description.`,
    )
  }
  if (revolutLink) {
    if (amountLabel && bookingRef) {
      notes.push(
        isEl
          ? `Για πληρωμή με Revolut, συμπλήρωσε ποσό ${amountLabel} και στο πεδίο «Σημείωση» γράψε τον κωδικό κράτησης ${bookingRef}.`
          : `For Revolut, enter ${amountLabel} and add booking ref ${bookingRef} in the Note field.`,
      )
    } else if (amountLabel) {
      notes.push(
        isEl
          ? `Για πληρωμή με Revolut, συμπλήρωσε ποσό ${amountLabel}.`
          : `For Revolut, enter ${amountLabel}.`,
      )
    } else if (bookingRef) {
      notes.push(
        isEl
          ? `Για πληρωμή με Revolut, στο πεδίο «Σημείωση» γράψε τον κωδικό κράτησης ${bookingRef}.`
          : `For Revolut, add booking ref ${bookingRef} in the Note field.`,
      )
    }
  }
  ...
  const bodyHtml = `
    <div style="margin:20px 0 0;padding:16px;background:${colors.outerBg};border:1px solid ${colors.cardBorder};border-radius:12px;">
      ...
      ${stripeHint}
      ${notes.map((note) => `<p style="margin:12px 0 0;font-family:system-ui,sans-serif;font-size:13px;line-height:1.5;color:${colors.muted};">${escapeHtml(note)}</p>`).join('')}
    </div>`
...
function buildMooveProgramPaymentAlternativesBlock({
  locale,
  stripeLink,
  paypalLink,
  revolutLink,
  ...
  const isEl = locale === 'el'
  const colors = mooveBrand.colors
  const hasAnyLink = Boolean(stripeLink || paypalLink || revolutLink)
  const purchaseRef = purchaseId ? formatBookingRef(purchaseId) : ''
  ...
  const note = isEl
    ? `Στην περιγραφή πληρωμής γράψε τον κωδικό παραγγελίας ${purchaseRef}.`
    : `Include order ref ${purchaseRef} in the payment description.`
  ...
  const bodyHtml = `
    <div style="margin:20px 0 0;padding:16px;background:${colors.outerBg};border:1px solid ${colors.cardBorder};border-radius:12px;">
      <p style="margin:0 0 8px;font-family:system-ui,sans-serif;font-size:12px;font-weight:600;letter-spacing:0.1em;text-transform:uppercase;color:${colors.muted};">${escapeHtml(altTitle)}</p>
      ${amountLabel ? `<p style="margin:0 0 8px;font-family:system-ui,sans-serif;font-size:14px;color:${colors.text};"><strong>${escapeHtml(amountLabel)}</strong></p>` : ''}
      <p style="margin:0 0 8px;font-family:system-ui,sans-serif;font-size:13px;line-height:1.5;color:${colors.muted};">${escapeHtml(note)}</p>
      <div>${links.join('')}</div>
    </div>`

  const bodyTextParts = [note]
  if (amountLabel) bodyTextParts.unshift(amountLabel)
  if (paypalLink) bodyTextParts.push(`PayPal: ${paypalLink}`)
  if (revolutLink) bodyTextParts.push(`Revolut: ${revolutLink}`)
...
/** Customer — Moove program order pending payment */
export function buildMooveProgramPaymentEmail({
  programName,
  purchaseId,
  amountEur,
  stripeLink,
  ...
  const isEl = locale === 'el'
  const amountLabel = amountEur ? `${Math.round(Number(amountEur))}€` : null
  const paymentAlt = buildMooveProgramPaymentAlternativesBlock({
    locale,
    stripeLink,
    paypalLink,
    revolutLink,
  ...
  return mooveTemplate({
    locale,
    subject: isEl
      ? `Moove — παραγγελία προγράμματος & πληρωμή`
      : `Moove — program order & payment`,
    preheader: isEl
      ? `Ολοκλήρωσε την πληρωμή για ${programName}`
      : `Complete payment for ${programName}`,
    title: isEl ? 'Η παραγγελία σου καταχωρήθηκε' : 'Your order is registered',
    greeting: isEl ? 'Γεια σου,' : 'Hi,',
    intro: isEl
      ? 'Ολοκλήρωσε την πληρωμή για να λάβεις email με το link πρόσβασης στο πρόγραμμα (Stripe, PayPal ή Revolut).'
      : 'Complete payment to receive an email with your program access link (Stripe, PayPal, or Revolut).',
    details: [
      { label: isEl ? 'Πρόγραμμα' : 'Program', value: programName },
      ...(amountLabel ? [{ label: isEl ? 'Τιμή' : 'Amount', value: amountLabel }] : []),
      {
        label: isEl ? 'Κωδικός παραγγελίας' : 'Order ref',
        value: formatBookingRef(purchaseId),
      },
    ],
    bodyHtml: paymentAlt.bodyHtml,
    bodyText: paymentAlt.bodyText,
    ctaHref: stripeLink || undefined,
    ctaLabel: stripeLink ? (isEl ? 'Πληρωμή μέσω Stripe' : 'Pay with Stripe') : undefined,
  })
}

/** Customer — Moove program access granted */
export function buildMooveProgramAccessEmail({ programName, accessUrl, locale = 'el' }) {
  const isEl = locale === 'el'

  return mooveTemplate({
    locale,
    subject: isEl ? `Moove — πρόσβαση στο πρόγραμμα` : `Moove — your program access`,
    preheader: isEl ? `Το πρόγραμμα ${programName} είναι έτοιμο` : `Your program ${programName} is ready`,
    title: isEl ? 'Η πρόσβαση στο πρόγραμμα είναι ενεργή' : 'Your program access is ready',
    greeting: isEl ? 'Γεια σου,' : 'Hi,',
    intro: isEl
      ? 'Η πληρωμή ολοκληρώθηκε. Κάνε κλικ στο κουμπί για να δεις το πρόγραμμα με ασκήσεις και videos.'
      : 'Payment completed. Click the button to view your program with exercises and videos.',
    details: [{ label: isEl ? 'Πρόγραμμα' : 'Program', value: programName }],
    ctaHref: accessUrl,
    ctaLabel: isEl ? 'Άνοιγμα προγράμματος' : 'Open program',
  })
}

/** Admin — new Moove program order */
export function buildMooveProgramOrderNotifyEmail({
  email,
  programName,
  purchaseId,
  amountEur,
  ...
  const details = [
    { label: 'Email', value: email, isLink: true },
    { label: isEl ? 'Πρόγραμμα' : 'Program', value: programName },
    { label: isEl ? 'Κωδικός παραγγελίας' : 'Order ref', value: formatBookingRef(purchaseId) },
    { label: isEl ? 'Κατάσταση' : 'Status', value: isEl ? 'Εκκρεμεί πληρωμή' : 'Pending payment' },
  ]

  if (amountLabel) {
    details.push({ label: isEl ? 'Τιμή' : 'Amount', value: amountLabel })
  }
  if (stripeLink) {
    details.push({
      label: isEl ? 'Stripe' : 'Stripe',
      value: isEl ? 'Άνοιγμα συνδέσμου' : 'Open link',
      isLink: true,
      href: stripeLink,
    })
  ...
  return mooveTemplate({
    locale,
    subject: isEl
      ? `Νέα παραγγελία προγράμματος — ${programName}`
      : `New program order — ${programName}`,
    preheader: `${email} · ${programName}`,
    title: isEl ? 'Νέα παραγγελία προγράμματος' : 'New program order',
    intro: isEl
      ? 'Νέα παραγγελία έτοιμου προγράμματος Moove.'
      : 'A new Moove ready-made program order was placed.',
    details,
    ctaHref: `${mooveBrand.siteUrl()}/posing/admin?tab=programs`,
    ctaLabel: isEl ? 'Admin panel' : 'Admin panel',
  })
}

function buildNutritionPaymentAlternativesBlock({
  locale,
  stripeLink,
  paypalLink,
  revolutLink,
  ...
  const colors = mooveBrand.colors
  const hasAnyLink = Boolean(stripeLink || paypalLink || revolutLink)
  const orderRef = orderId ? formatBookingRef(orderId) : ''
  ...
  const note = isEl
    ? `Στην περιγραφή πληρωμής γράψε τον κωδικό παραγγελίας ${orderRef}.`
    : `Include order ref ${orderRef} in the payment description.`
  ...
  if (stripeLink) {
    links.push(
      `<a href="${escapeHtml(stripeLink)}" style="${linkStyle}background:${colors.espresso};color:${colors.lime};">Stripe</a>`,
    )
  }
  if (paypalLink) {
    links.push(
      `<a href="${escapeHtml(paypalLink)}" style="${linkStyle}background:#003087;color:#fff;">PayPal</a>`,
    )
  }
  if (revolutLink) {
    links.push(
      `<a href="${escapeHtml(revolutLink)}" style="${linkStyle}background:#191C1F;color:#fff;">Revolut</a>`,
    )
  }

  const bodyHtml = `<p style="margin:16px 0 0;font-family:system-ui,sans-serif;font-size:14px;line-height:1.6;color:${colors.muted};">${escapeHtml(note)}</p>${links.join('')}`
  const bodyText = [note, stripeLink && `Stripe: ${stripeLink}`, paypalLink && `PayPal: ${paypalLink}`, revolutLink && `Revolut: ${revolutLink}`]
    .filter(Boolean)
    .join('\n')
...
/** Customer — nutrition plan payment pending */
export function buildNutritionPaymentEmail({
  orderId,
  amountEur,
  stripeLink,
  paypalLink,
  ...
  const isEl = locale === 'el'
  const amountLabel = amountEur ? `${Math.round(Number(amountEur))}€` : null
  const paymentAlt = buildNutritionPaymentAlternativesBlock({
    locale,
    stripeLink,
    paypalLink,
    revolutLink,
  ...
  return mooveTemplate({
    locale,
    subject: isEl ? 'Moove — παραγγελία διατροφής & πληρωμή' : 'Moove — nutrition order & payment',
    preheader: isEl ? 'Ολοκλήρωσε την πληρωμή για το διατροφικό πλάνο' : 'Complete payment for your nutrition plan',
    title: isEl ? 'Η παραγγελία σου καταχωρήθηκε' : 'Your order is registered',
    greeting: isEl ? 'Γεια σου,' : 'Hi,',
    intro: isEl
      ? 'Ολοκλήρωσε την πληρωμή για να λάβεις το προσωποποιημένο διατροφικό πλάνο σε PDF στο email σου.'
      : 'Complete payment to receive your personalized nutrition plan as a PDF via email.',
    details: [
      { label: isEl ? 'Υπηρεσία' : 'Service', value: isEl ? 'Προσωποποιημένη διατροφή' : 'Personalized nutrition' },
      ...(amountLabel ? [{ label: isEl ? 'Τιμή' : 'Amount', value: amountLabel }] : []),
      { label: isEl ? 'Κωδικός παραγγελίας' : 'Order ref', value: formatBookingRef(orderId) },
    ],
    bodyHtml: paymentAlt.bodyHtml,
    bodyText: paymentAlt.bodyText,
  })
...
/** Customer — nutrition plan delivered */
export function buildNutritionPlanEmail({ name, locale = 'el' }) {
  const isEl = locale === 'el'
  const greeting = name
    ? isEl
      ? `Γεια σου ${name},`
  ...
  return mooveTemplate({
    locale,
    subject: isEl ? 'Moove — το διατροφικό σου πλάνο' : 'Moove — your nutrition plan',
    preheader: isEl ? 'Το PDF είναι συνημμένο στο email' : 'Your PDF is attached to this email',
    title: isEl ? 'Το διατροφικό πλάνο είναι έτοιμο' : 'Your nutrition plan is ready',
    greeting,
    intro: isEl
      ? 'Βρήκες συνημμένο το προσωποποιημένο διατροφικό πλάνο σου σε PDF. Κράτα το για αναφορά κατά τη διάρκεια του προγράμματός σου.'
      : 'Your personalized nutrition plan is attached as a PDF. Keep it handy throughout your program.',
    bodyHtml: isEl
      ? '<p style="margin:16px 0 0;font-size:13px;color:#666;">Δεν αποτελεί ιατρική ή διαιτολογική συμβουλή.</p>'
      : '<p style="margin:16px 0 0;font-size:13px;color:#666;">This is not medical or dietary advice.</p>',
    bodyText: isEl
      ? 'Δεν αποτελεί ιατρική ή διαιτολογική συμβουλή.'
      : 'This is not medical or dietary advice.',
  })
}

/** Admin — new nutrition order */
export function buildNutritionOrderNotifyEmail({
  email,
  orderId,
  amountEur,
  stripeLink,
  ...
  const details = [
    { label: 'Email', value: email, isLink: true },
    { label: isEl ? 'Τύπος' : 'Type', value: typeLabel },
    { label: isEl ? 'Κωδικός παραγγελίας' : 'Order ref', value: formatBookingRef(orderId) },
    { label: isEl ? 'Κατάσταση' : 'Status', value: isEl ? 'Εκκρεμεί πληρωμή' : 'Pending payment' },
  ]

  if (amountLabel) details.push({ label: isEl ? 'Τιμή' : 'Amount', value: amountLabel })
  if (stripeLink) {
    details.push({
      label: 'Stripe',
      value: isEl ? 'Άνοιγμα συνδέσμου' : 'Open link',
  ...
  return mooveTemplate({
    locale,
    subject: isEl ? 'Νέα παραγγελία διατροφής' : 'New nutrition order',
    preheader: email,
    title: isEl ? 'Νέα παραγγελία διατροφής' : 'New nutrition order',
    intro: isEl ? 'Νέα παραγγελία προσωποποιημένης διατροφής Moove.' : 'A new Moove personalized nutrition order was placed.',
    details,
    ctaHref: `${mooveBrand.siteUrl()}/posing/admin?tab=nutrition`,
    ctaLabel: isEl ? 'Admin panel' : 'Admin panel',
  })
}

/** Admin — nutrition plan generation failed */
export function buildNutritionGenerationFailedEmail({ orderId, email, error, locale = 'el' }) {
  const isEl = locale === 'el'

  return mooveTemplate({
    locale,
    subject: isEl ? 'Αποτυχία δημιουργίας διατροφικού πλάνου' : 'Nutrition plan generation failed',
    preheader: formatBookingRef(orderId),
    title: isEl ? 'Αποτυχία δημιουργίας πλάνου' : 'Plan generation failed',
    intro: isEl
      ? 'Δεν ήταν δυνατή η αυτόματη δημιουργία διατροφικού πλάνου.'
      : 'Automatic nutrition plan generation failed.',
    details: [
      { label: 'Email', value: email, isLink: true },
      { label: isEl ? 'Κωδικός' : 'Ref', value: formatBookingRef(orderId) },
      { label: isEl ? 'Σφάλμα' : 'Error', value: error ?? 'unknown' },
    ],
    ctaHref: `${mooveBrand.siteUrl()}/posing/admin?tab=nutrition`,
    ctaLabel: isEl ? 'Admin panel' : 'Admin panel',
  })
